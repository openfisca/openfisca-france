...
def add_member(entity, **variables_value_by_name):
    ...
    for variable_name, variable_holder in entity.holder_by_name.items():
        ...
        if column.definition_period is ETERNITY:
            variable_holder._array = np.hstack(
                (variable_holder._array, [column.default_value])
            )
        else:
    ...
    if entity.is_persons_entity:
        for other_entity in simulation.entity_by_key_singular.itervalues():
            if not other_entity.is_persons_entity:
                ...
                value_by_name.setdefault(
                    other_entity.index_for_person_variable_name, other_entity.count - 1
                )
                role = value_by_name.get(other_entity.role_for_person_variable_name)
                assert (
                    role is not None
                ), "Missing role {} in person arguments: {}".format(
