...
def trace(
    ...
    api_url=u"http://api.openfisca.fr",
    json_dumped_file=None,
):

    scenario_json = scenario.to_json()
    simulation_json = {"scenarios": [scenario_json], "variables": variables}
    ...
    if json_dumped_file:
        with io.open(json_dumped_file, "w", encoding="utf-8") as f:
            f.write(
                unicode(
                    json.dumps(
                        scenario_json, ensure_ascii=False, encoding="utf-8", indent=2
                    )
                )
            )
