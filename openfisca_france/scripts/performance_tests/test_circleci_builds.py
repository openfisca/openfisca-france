...
def get_master_branch_performance():
    ...
    for response in response_python:
        ...
        if job_status == "success" and (
            job_name == "build_python2" or job_name == "build_python3"
        ):
            builds.append(response["build_time_millis"])

    if len(builds) < MIN_NUMBER_OF_MASTER_BUILDS:
        logging.warning(
            "Too few circle builds on master branch : {} builds".format(len(builds))
        )
        exit(1)
    else:
        logging.info(
            "The master branch has {} builds available for analysis.".format(
                len(builds)
            )
        )
...
def get_current_build_performance(python2_build_number, python3_build_number):
    ...
    for build in builds:
        api_url = "https://circleci.com/api/v1.1/project/github/openfisca/openfisca-france/{}".format(
            build
        )
        response = requests.get(api_url)
        if response.status_code == 200:
...
current_build = get_current_build_performance(
    python2_build_number, python3_build_number
)
current_master_ratio = (
    abs(master_branch["mean"] - current_build) / master_branch["mean"] * 100
)

if current_build > master_branch["mean"] * (1. + (MAX_RATIO_STANDARD_DEVIATION / 100.)):
    sys.exit(
        "This build makes the test performance more than {}% worst : {}%".format(
            MAX_RATIO_STANDARD_DEVIATION, current_master_ratio
        )
    )

elif current_build > master_branch["mean"] + master_branch["standard_deviation"]:
    sys.exit(
        "This build may be making the test performance worst : {}%".format(
            current_master_ratio
        )
    )
