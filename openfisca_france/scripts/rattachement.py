...
def split(scenario):
    ...
    for pac_index, pac_id in enumerate(foyer_fiscal.pop("personnes_a_charge")):
        ...
        if (
            18 <= age < (21 + 4 * (pac["activite"] == TypesActivite.etudiant))
    ...
    for i in range(0, n):
        ...
        scenarios[i].test_case["foyers_fiscaux"][0]["personnes_a_charge"] = (
            foyers_possibles[i] + detachements_impossibles
        )
        for jeune in rattachements_possibles:
            if jeune not in foyers_possibles[i]:
                scenarios[i].test_case["foyers_fiscaux"][j] = {
                    "declarants": [jeune],
                    "personnes_a_charge": [],
                }
                j += 1
...
def define_scenario(year):
    ...
    scenario.init_single_entity(
        ...
        enfants=[
            dict(activite=u"etudiant", date_naissance="1992-02-01"),
            dict(activite=u"etudiant", date_naissance="2000-04-17"),
        ],
