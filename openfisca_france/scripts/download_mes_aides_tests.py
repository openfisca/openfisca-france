...
tests_dir_path = os.path.abspath(
    os.path.join(source_file_dir_name, "..", "tests", "mes-aides.gouv.fr")
)
...
def fetch_json(url):
    r = requests.get(
        url, headers={"User-Agent": "OpenFisca-Script {}".format(app_name)}
    )
    return r.json()


def fetch_situation(api_base_url, situation_id):
    reference = u"{}/api/situations/{}/openfisca-request".format(
        api_base_url, situation_id
    )
...
def iter_yaml_items(api_base_url, test):
    situation_json = fetch_situation(
        api_base_url=api_base_url, situation_id=test["scenario"]["situationId"]
    )
    log.info(
        u"ID: {} [{}] {}".format(
            test["_id"], u", ".join(sorted(test["keywords"])), test["name"]
        )
    )
    ...
    description_style = (
        "|" if description is not None and len(description.split("\n")) > 1 else None
    )
...
def main():
    ...
    parser.add_argument(
        "--api-base-url",
        default=u"https://mes-aides.gouv.fr",
        help=u"Ludwig API base URL",
    )
    parser.add_argument(
        "--output-dir", default=tests_dir_path, help=u"Where to write the tests"
    )
    parser.add_argument(
        "--tests-json", default=None, help=u"Do not download tests, use given file"
    )
    parser.add_argument(
        "--test-ids", default=None, help=u"Download only those IDs", nargs="+"
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    for test_json in tests_json:
        ...
        assert test_json["currentStatus"] in (
            "accepted-exact",
            "accepted-2pct",
            "accepted-10pct",
            "rejected",
        ...
        if any(
            "expectedValue" in result
            and "status" in result
            and result["status"] not in ("accepted-exact", "accepted-2pct")
            for result in last_execution["results"]
        ):
            log.info(
                u'Test "{}" doesn\'t return the expected value (yet), so skip it.'.format(
                    test_json["_id"]
                )
            )
            continue
        ...
        with open(yaml_file_path, "w") as yaml_file:
            for yaml_key, yaml_value, default_style in iter_yaml_items(
                api_base_url=args.api_base_url, test=test_json
            ):
                yaml.safe_dump(
                    {yaml_key: yaml_value},
                    yaml_file,
