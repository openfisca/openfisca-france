...
note_names = (u"Notes", u"Notes bis")
reference_names = (
    u"Parution au JO",
    u"Références BOI",
...
def build_tree_from_yaml_clean(yaml_dir):
    ...
    for yaml_dir_encoded, _, filenames_encoded in os.walk(yaml_dir):
        for filename_encoded in sorted(filenames_encoded):
            ...
            relative_file_path_encoded = yaml_file_path_encoded[len(yaml_dir) :].lstrip(
                os.sep
            )
            ...
            if rows is None:
                log.info(
                    u'  Skipping file {} without "Valeurs"'.format(relative_file_path)
                )
                continue
            row_by_start = {}
            for row in rows:
            ...
            for start, row in sorted_row_by_start:
                ...
                for name, child in row.items():
                    ...
                    start_relative_ipp_paths.extend(
                        (name,) + tuple(path) for path, value in iter_ipp_values(child)
                    )
            ...
            def compare_relative_ipp_paths(x, y):
                ...
                for relative_ipp_paths in relative_ipp_paths_by_start.itervalues():
                    try:
                        return cmp(
                            relative_ipp_paths.index(x), relative_ipp_paths.index(y)
                        )
                    except ValueError:
            ...
            sorted_relative_ipp_paths = sorted(
                unsorted_relative_ipp_paths, cmp=compare_relative_ipp_paths
            )
            ...
            for start, row in sorted_row_by_start:
                for relative_ipp_path in sorted_relative_ipp_paths:
                    ...
                    ipp_path = [
                        fragment
                        if fragment in ("RENAME", "TRANCHE", "TYPE")
                    ...
                    if sub_tree:
                        ...
                        if previous_leaf["value"] == value:
                            ...
                            continue
                    sub_tree.append(dict(start=start, value=value))
    return tree
...
def prepare_xml_values(name, leafs):
    leafs = list(reversed([leaf for leaf in leafs if leaf["value"] is not None]))
    format = None
    type = None
    for leaf in leafs:
        value = leaf["value"]
            ...
            if len(split_value) == 2 and split_value[1] in (
                ...
                if unit == u"%":
                    if format is None:
                        ...
                        log.warning(
                            u"Non constant percent format {} in {}: {}".format(
                                format, name, leafs
                            )
                        )
                    ...
                    if format is None:
                        ...
                        log.warning(
                            u"Non constant float format {} in {}: {}".format(
                                format, name, leafs
                            )
                        )
                        return None, format, type
                    if type is None:
                        ...
                        log.warning(
                            u"Non constant monetary type {} in {}: {}".format(
                                type, name, leafs
                            )
                        )
                        return None, format, type
...
def ipp_node_to_element(name, node):
    ...
    if isinstance(node, dict):
        if node.get("TYPE") == u"BAREME":
            bareme_element = etree.Element(
                "BAREME",
                attrib=dict(code=strings.slugify(name, separator=u"_"), origin=u"ipp"),
            )
            for slice_name in node.get("SEUIL", {}).keys():
                tranche_element = etree.Element(
                    "TRANCHE",
                    attrib=dict(code=strings.slugify(slice_name, separator=u"_")),
                )

                seuil_element = etree.Element("SEUIL")
                values, format, type = prepare_xml_values(
                    name, node.get("SEUIL", {}).get(slice_name, [])
                )
                ...
                values, format, type = prepare_xml_values(
                    name, node.get("TAUX", {}).get(slice_name, [])
                )
...
def transform_values_to_element_children(values, element):
    element.extend(
        map(
            lambda value: etree.Element(
                "VALUE",
                attrib=dict(
                    deb=value["start"].isoformat(), valeur=unicode(value["value"])
                ),
            ),
            values,
        )
...
def write_xml_file(xml_dir, file_name, element):
    ...
    element_tree.write(
        os.path.join(xml_dir, file_name), encoding="utf-8", pretty_print=True
    )
...
def transform(yaml_clean_dir, xml_dir):
    ...
    transform_ipp_tree(
        ipp_tree
    )  # User-written transformations which modify `ipp_tree`.
    ipp_root_element = ipp_node_to_element(u"root", ipp_tree)

    for child_element in ipp_root_element:
        write_xml_file(
            xml_dir, "{}.xml".format(child_element.attrib["code"]), child_element
        )
