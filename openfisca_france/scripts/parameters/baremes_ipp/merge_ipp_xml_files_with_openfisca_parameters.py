...
def merge_elements(openfisca_element, ipp_element, path=[]):
    ...
    assert ipp_element.attrib["code"] == openfisca_element.attrib["code"], (
        openfisca_element,
        ipp_element,
    )
    assert ipp_element.tag == openfisca_element.tag, error_at_path(
        u"OpenFisca element {!r} differs from IPP element {!r}".format(
            openfisca_element.tag, ipp_element.tag
        ).encode("utf-8")
    )
    ...
    if openfisca_element.tag == "NODE":
        ipp_children_by_code = {child.attrib["code"]: child for child in ipp_element}
        for openfisca_child in openfisca_element:
            ...
            if code in ipp_children_by_code.keys():
                ...
                merge_elements(
                    openfisca_child,
                    ipp_child,
                    path + [openfisca_element.attrib["code"]],
                )
        ...
        for index, openfisca_tranche in enumerate(openfisca_element):
            ...
            ipp_tranche_children_by_tag = {child.tag: child for child in ipp_tranche}
...
def merge_ipp_xml_files_with_openfisca_parameters(ipp_xml_dir):
    ...
    openfisca_xml_tree_by_file_name = get_xml_tree_by_file_name(
        xmlschema, openfisca_file_paths
    )
    ...
    for (
        openfisca_file_name,
        openfisca_xml_tree,
    ) in openfisca_xml_tree_by_file_name.items():
        ...
        if ipp_xml_tree is not None:
            ...
            merge_elements(
                openfisca_xml_root_element, ipp_xml_root_element
            )  # Mutates `openfisca_xml_root_element`
