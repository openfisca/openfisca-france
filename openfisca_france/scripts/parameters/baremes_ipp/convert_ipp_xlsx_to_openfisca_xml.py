...
from merge_ipp_xml_files_with_openfisca_parameters import (
    merge_ipp_xml_files_with_openfisca_parameters
)
...
def main():
    ...
    parser.add_argument(
        "--ref-ipp",
        default=None,
        help=u"Reference of the repository ipp-tax-and-benefit-tables-xlsx to use",
    )
    parser.add_argument(
        "--zip-url", default=None, help=u"URL of the ZIP file to download"
    )
    parser.add_argument(
        "--tmp-dir", default=None, help=u"Where to write intermediary files"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help=u"Merge the generated XML with the OpenFisca France parameters",
    )
    ...
    if args.zip_url:
        ...
        zip_url = u"https://framagit.org/french-tax-and-benefit-tables/ipp-tax-and-benefit-tables-xlsx/repository/archive.zip?ref={}".format(
            ref_ipp
        )
    ...
    if not cmd_exists("ssconvert"):
        log.error(
            u'Command "ssconvert" must be installed. It is provided by the "gnumeric" spreadsheet. '
    ...
    tmp_dir = (
        tempfile.mkdtemp(prefix="baremes-ipp-")
    ...
    xlsx_dir_path = glob.glob(
        os.path.join(tmp_dir, "ipp-tax-and-benefit-tables-xlsx-*")
    )[0]
    ...
    for xlsx_file_name in os.listdir(xlsx_dir_path):
        ...
        target_path = os.path.join(
            xls_dir_path, "{}.xls".format(os.path.splitext(xlsx_file_name)[0])
        )
