...
def fixed_bases_tax_scale(base_by_slice_name, rates_tree, null_rate_base=None):
    ...
    for slice_name, bracket in rates_tree.items():
        if isinstance(bracket, (float, int)):
            rates_bracket_item = dict(start=first_start, value=str(bracket))
    ...
    if null_rate_base is not None:
        ...
        for bracket in rates_tree.itervalues():
            if first_start < bracket[0]["start"]:
                bracket.insert(0, dict(start=first_start, value="0"))
            for item in bracket:
                if item["value"] is None:
                    item["value"] = "0"

        rates_bracket_null_item = dict(start=first_start, value="0")
    ...
    for slice_name, rates_bracket in sorted(rates_tree.items()):
        bases_bracket_item = dict(
            start=rates_bracket[0]["start"], value=str(base_by_slice_name[slice_name])
        )
...
def tax_scale(bases_tree, rates_tree):
    """Crée un barème qui sera transformé en un élément XML <BAREME> par le script de fusion."""
    return dict(TYPE="BAREME", SEUIL=bases_tree, TAUX=rates_tree)
...
def transform_ipp_tree(root):
    ...
    root["impot_revenu"] = impot_revenu = root.pop(
        "baremes_ipp_impot_revenu_income_tax"
    )
    #
    ...
    maximum_de_l_abattement_pour_frais_professionnels = tspr.pop(
        "maximum_de_l_abattement_pour_frais_professionnels"
    )
    ...
    minimum_de_l_abattement_pour_frais_professionnels = tspr.pop(
        "minimum_de_l_abattement_pour_frais_professionnels"
    )
    ...
    abatpro["min2"] = minimum_de_l_abattement_pour_frais_professionnels.pop(
        "allocations_chomage"
    )
    assert (
        not maximum_de_l_abattement_pour_frais_professionnels
    ), maximum_de_l_abattement_pour_frais_professionnels
    assert (
        not minimum_de_l_abattement_pour_frais_professionnels
    ), minimum_de_l_abattement_pour_frais_professionnels
    #
    impot_revenu["abattements_rni"] = abattements_rni = impot_revenu.pop("abat_rni")
    abattements_rni["enfant_marie"] = abattements_rni.pop(
        "abattement_pour_rattachement_d_enfants_maries"
    )
    abattements_rni[
        "personne_agee_ou_invalide"
    ] = personne_agee_ou_invalide = abattements_rni.pop(
    ...
    #
    impot_revenu["charges_deductibles"] = charges_deductibles = impot_revenu.pop(
        "charg_deduc"
    )
    ...
    charges_deductibles[
        "compte_epargne_codev"
    ] = compte_epargne_codev = charges_deductibles.pop(
        "reduction_pour_versement_compte_epargne_codev"
    )
    compte_epargne_codev["plafond_pct_rng"] = compte_epargne_codev.pop(
        "plafond_en_du_revenu_net_global"
    )
    charges_deductibles["grosses_reparations"] = charges_deductibles.pop(
        "deductions_pour_grosses_reparations"
    )
    charges_deductibles["pensions_alimentaires"] = charges_deductibles.pop(
        "deduction_pour_pensions_alimentaires"
    )
    charges_deductibles[
        "pertes_en_capital_societes_nouvelles"
    ] = pertes_en_capital_societes_nouvelles = charges_deductibles.pop(
        "deductions_pour_pertes_en_capital_societes_nouvelles"
    )
    pertes_en_capital_societes_nouvelles[
        "plafond_cb"
    ] = pertes_en_capital_societes_nouvelles.pop("plafond_1")
    pertes_en_capital_societes_nouvelles[
        "plafond_da"
    ] = pertes_en_capital_societes_nouvelles.pop("plafond_2")
    ...
    micro["microfoncier"] = microfoncier = micro.pop(
        "abattement_pour_le_regime_microfoncier"
    )
    ...
    specialbnc["marchandises"] = dict(
        max=specialbnc.pop("plafond_de_recettes_marchandises")
    )
    ...
    produits_des_assurances_vies_et_assimiles = rvcm[
        "produits_des_assurances_vies_et_assimiles"
    ]
    rvcm["abat_assvie"] = abat_assvie = produits_des_assurances_vies_et_assimiles.pop(
        "abattement"
    )
    revenus_de_capitaux_mobiliers_dividendes = rvcm[
        "revenus_de_capitaux_mobiliers_dividendes"
    ]
    rvcm["abatmob"] = abatmob = revenus_de_capitaux_mobiliers_dividendes.pop(
        "abattement_forfaitaire"
    )
    impot_revenu["rvcm"].update(rvcm.pop("revenus_de_capitaux_mobiliers_dividendes"))
    rvcm[
        "taux_abattement_capitaux_mobiliers"
    ] = taux_abattement_capitaux_mobiliers = rvcm.pop("taux_de_l_abattement")
    impot_revenu["reductions_impots"] = reductions_impots = dict()
    reductions_impots = impot_revenu["reductions_impots"]
    reductions_impots["salarie_domicile"] = salarie_domicile = impot_revenu.pop(
        "sal_dom"
    )
    # Plus-values
    impot_revenu["plus_values"] = plus_values = impot_revenu.pop("pv")
    impot_revenu["plus_values"].update(
        plus_values.pop(
    ...
    dons[
        "taux_max_dons_partipo"
    ] = taux_max_dons_partipo = dons_aux_partis_politiques.pop("plafond")
    reductions_impots["spfcpi"] = spfcpi = impot_revenu.pop("fcp")
    reductions_impots["spfcpi"].update(
        spfcpi.pop("reduction_d_impot_pour_souscriptions_de_parts_de_fcpi_ou_fip")
    )
    ...
    ppe["seuil2"] = seuil2 = ppe.pop(
        "revenu_d_activite_individuel_permettant_d_obtenir_la_ppe_a_taux_plein_cas_general"
    )
    ppe["seuil3"] = seuil3 = ppe.pop("revenu_d_activite_individuel_maximum_cas_general")
    ppe["seuil4"] = seuil4 = ppe.pop(
        "revenu_d_activite_individuel_permettant_d_obtenir_la_ppe_a_taux_plein_couples_mono_revenus"
    )
    ppe["seuil5"] = seuil5 = ppe.pop(
        "revenu_d_activite_individuel_maximum_couples_mono_emploi_et_parents_isoles"
    )
    ...
    impot_revenu["plafond_qf"].update(
        plafond_qf.pop("plafond_des_avantages_procures_par_demi_part_de_qf")
    )
    ...
    plafond_qf["celib_enf"] = plafond_qf.pop(
        "part_pour_le_1er_enfant_des_parents_isoles"
    )
    ...
    credit_d_impot_pour_frais_de_garde_d_enfants = garext[
        "credit_d_impot_pour_frais_de_garde_d_enfants"
    ]
    credits_impot["garext"].update(
        garext.pop("credit_d_impot_pour_frais_de_garde_d_enfants")
    )

    # contribution haut revenus
    taxe_hr = impot_revenu["taxe_hr"]
    impot_revenu["taxe_hr"].update(
        taxe_hr.pop("contribution_exceptionnelle_sur_les_hauts_revenus")
    )
    ...
    ecodev["taux_plafond"] = taux_plafond = ecodev.pop(
        "plafond_en_du_revenu_net_global"
    )

    # reduction impot emprunt
    reductions_impots["intemp"] = intemp = impot_revenu.pop("habitat_princ_reduc")
    reduction_d_impot_pour_interets_d_emprunt_habitat = intemp[
        "reduction_d_impot_pour_interets_d_emprunt_habitat"
    ]
    reduction_d_impot_pour_interets_d_emprunt_habitat[
        "taux1"
    ] = reduction_d_impot_pour_interets_d_emprunt_habitat.pop("taux")
    reductions_impots["intemp"].update(
        intemp.pop("reduction_d_impot_pour_interets_d_emprunt_habitat")
    )
    ...
    reductions_impots["daepad"].update(
        daepad.pop(
    ...
    depenses_d_investissement_forestier = invfor.pop(
        "depenses_d_investissement_forestier"
    )
    invfor.update(
        {
    ...
    agff["employeur"]["non_cadre"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_1=0, tranche_2=1),
        null_rate_base=3,
        rates_tree=dict(
            tranche_1=tranche_1_a_employeur, tranche_2=tranche_2.pop("employeur")
        ),
    )
    agff["employeur"]["cadre"] = fixed_bases_tax_scale(
        ...
        null_rate_base=4,
        rates_tree=dict(
            tranche_1=tranche_1_a_employeur, tranche_2=tranche_b.pop("employeur")
        ),
    )
    agff["salarie"] = dict()
    agff["salarie"]["non_cadre"] = fixed_bases_tax_scale(
        ...
        null_rate_base=3,
        rates_tree=dict(
            tranche_1=tranche_1_a_salarie, tranche_2=tranche_2.pop("salarie")
        ),
    )
    agff["salarie"]["cadre"] = fixed_bases_tax_scale(
        ...
        null_rate_base=4,
        rates_tree=dict(
            tranche_1=tranche_1_a_salarie, tranche_2=tranche_b.pop("salarie")
        ),
    )
    ...
    agirc["employeur"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_a=0, tranche_b=1, tranche_c=4),
        null_rate_base=8,
        rates_tree=dict(
            tranche_a=0,
    ...
    ags["employeur"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_a_b=0),
        null_rate_base=4,
        rates_tree=dict(tranche_a_b=employeurs.pop("sous_4_pss")),
    )
    ...
    apec["employeur_avant_2011"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_a=0, tranche_b=1),
        null_rate_base=4,
        rates_tree=dict(tranche_a=0, tranche_b=tranche_b.pop("employeur")),
    )
    apec["employeur"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_a_b=0),
        null_rate_base=4,
        rates_tree=dict(tranche_a_b=tranche_a_b.pop("employeur")),
    )
    apec["salarie_avant_2011"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_a=0, tranche_b=1),
        null_rate_base=4,
        rates_tree=dict(tranche_a=0, tranche_b=tranche_b.pop("salarie")),
    )
    apec["salarie"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_a_b=0),
        null_rate_base=4,
        rates_tree=dict(tranche_a_b=tranche_a_b.pop("salarie")),
    )
    assert not tranche_b, tranche_b
    assert not tranche_a_b, tranche_a_b
    #
    cotisations_sociales["apprentissage"] = apprentissage = prelevements_sociaux.pop(
        "apprentissage"
    )
    employeur_tout_salaire = apprentissage.pop("employeur_tout_salaire")
    apprentissage["taxe_apprentissage"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_unique=0),
        rates_tree=dict(
    ...
    arrco_taux_effectifs_tranche_2 = taux_effectifs_salaries_employeurs.pop(
        "tranche_2_apres_1997"
    )
    ...
    contributions["casa"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_unique=0),
        rates_tree=dict(
            tranche_unique=casa.pop(
                "pensions_de_retraite_de_preretraite_et_d_invalidite"
            )
        ),
    )
    #
    ...
    cet["employeur"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(salaire_sous_8_pss=0),
        null_rate_base=8,
        rates_tree=dict(salaire_sous_8_pss=salaire_sous_8_pss.pop("employeur")),
    )
    cet["salarie"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(salaire_sous_8_pss=0),
        null_rate_base=8,
        rates_tree=dict(salaire_sous_8_pss=salaire_sous_8_pss.pop("salarie")),
    )
    ...
    ] = vieillesse_deplafonnee = vieillesse.pop("sur_tout_salaire")
    cotisations_sociales[
        "vieillesse_plafonnee"
    ] = vieillesse_plafonnee = vieillesse.pop("salaire_sous_plafond")
    vieillesse_deplafonnee["employeur"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_unique=0),
        rates_tree=dict(tranche_unique=vieillesse_deplafonnee.pop("employeurs")),
    )
    vieillesse_deplafonnee["salarie"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_unique=0),
        rates_tree=dict(tranche_unique=vieillesse_deplafonnee.pop("salaries")),
    )
    vieillesse_plafonnee["employeur"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_unique=0),
        null_rate_base=1,
        rates_tree=dict(tranche_unique=vieillesse_plafonnee.pop("employeurs")),
    )
    vieillesse_plafonnee["salarie"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_unique=0),
        null_rate_base=1,
        rates_tree=dict(tranche_unique=vieillesse_plafonnee.pop("salaries")),
    )
    ...
    cnracl_salarie["hors_nbi"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_unique=0),
        rates_tree=dict(tranche_unique=cnracl_salarie.pop("hors_nbi")),
    )
    cnracl_salarie["nbi"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_unique=0),
        rates_tree=dict(tranche_unique=cnracl_salarie.pop("nbi")),
    )
    cnracl["employeur"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_unique=0),
        rates_tree=dict(tranche_unique=cnracl.pop("employeurs")),
    )
    cnracl_node["atiacl"] = atiacl = cnracl_node.pop("atiacl")
    atiacl["employeur"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_unique=0),
        rates_tree=dict(tranche_unique=atiacl.pop("hors_nbi")),
    )
    # TODO: FCPPA
    cnracl_node["feh"] = feh = cnracl_node.pop("feh")
    feh["employeur"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_unique=0),
        rates_tree=dict(tranche_unique=feh.pop("hors_cl")),
    )
    #
    cotisations_sociales["construction"] = construction = prelevements_sociaux.pop(
        "construction"
    )
    construction_employeur_sur_tout_salaire = construction.pop(
        "employeur_sur_tout_salaire"
    )
    construction["construction_10_19"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_unique=0),
        rates_tree=dict(
            tranche_unique=construction_employeur_sur_tout_salaire.pop(
                "de_10_a_19_salaries"
            )
        ),
    )
    ...
    csg["remplacement"] = csg_remplacement = prelevements_sociaux.pop(
        "csg_remplacement"
    )
    ...
    cotisations_sociales["famille"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_unique=0),
        rates_tree=dict(tranche_unique=famille_sur_tout_salaire.pop("employeurs")),
    )
    ...
    fds["cotisation_exceptionnelle_solidarite"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_unique=0),
        null_rate_base=4,
        rates_tree=dict(tranche_unique=fds.pop("taux")),
    )
    #
    ...
    del fillon["entreprises_restees_aux_39_heures_hebdomadaires_au_30_06_2003"][
        "salaire_de_reference"
    ]
    del fillon["entreprises_ayant_signe_un_accord_de_rtt_avant_le_30_06_2003"][
        "salaire_de_reference"
    ]
    #
    ...
    fnal["tout_employeur"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(sous_pss=0),
        null_rate_base=1,
        rates_tree=dict(sous_pss=tout_employeur.pop("sous_pss")),
    )
    ...
    #
    cotisations_sociales["formation"] = formation = prelevements_sociaux.pop(
        "formation"
    )
    formation["employeur"] = employeur = formation.pop("employeur_tout_salaire")
    employeur["moins_de_10_salaries"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tout_salaire=0),
        rates_tree=dict(tout_salaire=employeur.pop("moins_de_10_salaries")),
    )
    employeur["de_10_a_19_salaries"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tout_salaire=0),
        rates_tree=dict(tout_salaire=employeur.pop("de_10_a_19_salaries")),
    )
    employeur["20_salaries_et_plus"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tout_salaire=0),
        rates_tree=dict(tout_salaire=employeur.pop("20_salaries_et_plus")),
    )
    ...
    ircantec["employeur"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_a=0, tranche_b_1_4_75_puis_1_a_8_pss=1),
        null_rate_base=8,
        rates_tree=dict(
            tranche_a=taux_effectifs["tranche_a"].pop("employeur"),
            tranche_b_1_4_75_puis_1_a_8_pss=taux_effectifs[
                "tranche_b_1_4_75_puis_1_a_8_pss"
            ].pop("employeur"),
        ),
    )
    ...
    mmid["employeur"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_a=0),
        rates_tree=dict(tranche_a=sur_tout_salaire.pop("employeurs")),
    )
    mmid["employeur2"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_a=0),
        null_rate_base=1,
        rates_tree=dict(tranche_a=salaire_sous_plafond.pop("employeurs")),
    )
    mmid["reduction_65_ans"] = salaire_sous_plafond.pop("reduction_65_ans")
    mmid["salarie"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tout_salaire=0),
        rates_tree=dict(tout_salaire=sur_tout_salaire.pop("salaries")),
    )
    mmid["salarie2"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_a=0),
        null_rate_base=1,
        rates_tree=dict(tranche_a=salaire_sous_plafond.pop("salaries")),
    )
    ...
    rafp["employeur"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_a=0),
        rates_tree=dict(tranche_a=taux_de_cotisation.pop("etat")),
    )
    rafp["salarie"] = fixed_bases_tax_scale(
        base_by_slice_name=dict(tranche_a=0),
        rates_tree=dict(tranche_a=taux_de_cotisation.pop("agent")),
    )
    ...
    modulation["plafond_tranche_1"] = modulation_ipp.pop(
        "plafond_de_ressources_tranche_1"
    )
    modulation["plafond_tranche_2"] = modulation_ipp.pop(
        "plafond_de_ressources_tranche_2"
    )
    ...
    af_dom = af["af_dom"]
    majoration_pour_le_premier_enfant_en_de_la_bmaf_dom = af_dom[
        "majoration_pour_le_premier_enfant_en_de_la_bmaf_dom"
    ]
    tranches_d_age = af_dom["tranches_d_age"]
    af_dom[
        "taux_1er_enf_tranche_2_dom"
    ] = (
        taux_1er_enf_tranche_2_dom
    ) = majoration_pour_le_premier_enfant_en_de_la_bmaf_dom.pop(
        "taux_de_la_tranche_2"
    )
    af_dom[
        "taux_1er_enf_tranche_1_dom"
    ] = (
        taux_1er_enf_tranche_1_dom
    ) = majoration_pour_le_premier_enfant_en_de_la_bmaf_dom.pop(
        "taux_de_la_tranche_1"
    )
    af_dom[
        "age_1er_enf_tranche_1_dom"
    ] = age_1er_enf_tranche_1_dom = tranches_d_age.pop(
        "age_debut_de_la_premiere_tranche"
    )
    af_dom[
        "age_1er_enf_tranche_2_dom"
    ] = age_1er_enf_tranche_2_dom = tranches_d_age.pop(
    ...
    af["majoration_enfants"] = majoration_enfants = af.pop(
        "majoration_pour_les_enfants_en_de_la_bmaf"
    )
    majoration_enfants = af["majoration_enfants"]
    af["majoration_enfants"].update(majoration_enfants.pop("allocation_forfaitaire"))
    majoration_enfants[
        "age_minimal_forfait"
    ] = age_minimal_forfait = majoration_enfants.pop("age_minimum")
    majoration_enfants[
        "taux_allocation_forfaitaire"
    ] = taux_allocation_forfaitaire = majoration_enfants.pop("taux")
    ...
    prestations["aes"]["complement_d_allocation"]["3e_categorie"] = [
        element
        for element in prestations["aes"]["complement_d_allocation"]["3e_categorie"]
    ...
    forfait_charges["cas_general"] = al_charge["personne_isolee_ou_menage_seul"].pop(
        "cas_general"
    )
    forfait_charges["majoration_par_enfant"] = al_charge.pop(
        "majoration_par_enfant_de_la_majoration_pour_charges_2"
    )
    ...
    al_param2["r1_en_du_rsa_socle"] = r1_en_du_rsa_socle = al_param2.pop(
        "r1_en_du_rsa_socle_1"
    )
    ...
    r1_en_du_rsa_socle[
        "majoration_enfant_a_charge_supp"
    ] = majoration_enfant_a_charge_supp = r1_en_du_rsa_socle.pop(
        "majoration_par_enfant_a_charge_supplementaire_en_du_rmi_de_base"
    )
    r1_en_du_rsa_socle["couple_sans_enf"] = couple_sans_enf = r1_en_du_rsa_socle.pop(
        "couples_en_du_rmi_de_base"
    )
    r1_en_du_rsa_socle[
        "personne_isolee_ou_couple_avec_1_enf"
    ] = personne_isolee_ou_couple_avec_1_enf = r1_en_du_rsa_socle.pop(
        "personnes_isolees_ou_couples_avec_1_enfant_en_du_rmi_de_base"
    )
    r1_en_du_rsa_socle[
        "personne_isolee_ou_couple_avec_2_enf"
    ] = personne_isolee_ou_couple_avec_2_enf = r1_en_du_rsa_socle.pop(
        "personnes_isolees_ou_couples_avec_2_enfants_en_du_rmi_de_base"
    )
    aides_logement["r1"] = r1 = al_param2.pop("r1_en_du_rsa_socle")

    al_param2["r2_ipp"] = r2_ipp = al_param2.pop("r2_en_de_la_bmaf_1")
    r2_ipp[
        "majoration_par_enf_supp_a_charge"
    ] = majoration_par_enf_supp_a_charge = r2_ipp.pop(
        "majoration_par_enfant_a_charge_supplementaire_en_de_la_bmaf_au_01_01_n_2"
    )
    r2_ipp[
        "personnes_isolees_ou_couples_avec_2_enf"
    ] = personnes_isolees_ou_couples_avec_2_enf = r2_ipp.pop(
    ...
    al_plaf_loc2[
        "taux_participation_fam_ipp"
    ] = taux_participation_fam_ipp = al_plaf_loc2.pop("tf")
    taux_participation_fam_ipp = al_plaf_loc2["taux_participation_fam_ipp"]
    taux_participation_fam_ipp[
        "taux_1_adulte"
    ] = taux_1_adulte = taux_participation_fam_ipp.pop("personnes_isolees")
    taux_participation_fam_ipp[
        "taux_2_adulte"
    ] = taux_2_adulte = taux_participation_fam_ipp.pop("couples_sans_enfant")
    taux_participation_fam_ipp[
        "taux_1_enf"
    ] = taux_1_enf = taux_participation_fam_ipp.pop(
        "personnes_seules_et_couples_avec_1_enfant"
    )
    taux_participation_fam_ipp[
        "taux_2_enf"
    ] = taux_2_enf = taux_participation_fam_ipp.pop(
        "personnes_seules_et_couples_avec_2_enfants"
    )
    taux_participation_fam_ipp[
        "taux_3_enf"
    ] = taux_3_enf = taux_participation_fam_ipp.pop(
        "personnes_seules_et_couples_avec_3_enfants"
    )
    taux_participation_fam_ipp[
        "taux_4_enf"
    ] = taux_4_enf = taux_participation_fam_ipp.pop(
        "personnes_seules_et_couples_avec_4_enfants"
    )
    taux_participation_fam_ipp[
        "taux_enf_supp"
    ] = taux_enf_supp = taux_participation_fam_ipp.pop(
        "variation_de_tf_par_enfant_supplementaire"
    )
    aides_logement[
        "taux_participation_fam"
    ] = taux_participation_fam = al_plaf_loc2.pop("taux_participation_fam_ipp")
    del taux_participation_fam_ipp
    al_plaf_loc2[
        "taux_participation_loyer_ipp"
    ] = taux_participation_loyer_ipp = al_plaf_loc2.pop("tl")
    taux_participation_loyer_ipp[
        "taux_tranche_1"
    ] = taux_tranche_1 = taux_participation_loyer_ipp.pop("tl_pour_la_1ere_tranche")
    taux_participation_loyer_ipp[
        "taux_tranche_2"
    ] = taux_tranche_2 = taux_participation_loyer_ipp.pop("tl_pour_la_2eme_tranche")
    taux_participation_loyer_ipp[
        "taux_tranche_3"
    ] = taux_tranche_3 = taux_participation_loyer_ipp.pop("tl_pour_la_3eme_tranche")
    aides_logement[
        "taux_participation_loyer"
    ] = taux_participation_loyer = al_plaf_loc2.pop("taux_participation_loyer_ipp")
    # par zone
    ...
    zone1["un_enfant"] = plafond_de_loyers_zone_1.pop(
        "personnes_seules_ou_couples_avec_1_enfant"
    )
    zone1["majoration_par_enf_supp"] = plafond_de_loyers_zone_1.pop(
        "majoration_par_enfant_supplementaire"
    )
    ...
    zone2["un_enfant"] = plafond_de_loyers_zone_2.pop(
        "personnes_seules_ou_couples_avec_1_enfant"
    )
    zone2["majoration_par_enf_supp"] = plafond_de_loyers_zone_2.pop(
        "majoration_par_enfant_supplementaire"
    )
    ...
    zone3["un_enfant"] = plafond_de_loyers_zone_3.pop(
        "personnes_seules_ou_couples_avec_1_enfant"
    )
    zone3["majoration_par_enf_supp"] = plafond_de_loyers_zone_3.pop(
        "majoration_par_enfant_supplementaire"
    )

    del taux_participation_loyer_ipp
    aides_logement["al_min"] = al_min = prestations.pop("al_min")
    al_min["montant_min_mensuel"] = montant_min_mensuel = al_min.pop(
        "montant_minimal_mensuel"
    )
    montant_min_mensuel[
        "montant_min_apl_al"
    ] = montant_min_apl_al = montant_min_mensuel.pop("apl_ou_al")
    ...
    aides_logement["taux"] = taux = al_plaf_loc2.pop(
        "montant_proportionnel_de_la_participation_minimale_po"
    )
    ...
    ars_cond["age_entree_primaire"] = age_entree_primaire = ars_cond.pop(
        "age_minimal_de_l_enfant_1"
    )
    ars_cond["age_sortie_lycee"] = age_sortie_lycee = ars_cond.pop(
        "age_maximal_de_l_enfant_2"
    )
    ars["age_entree_primaire"] = age_entree_primaire = ars_cond.pop(
        "age_entree_primaire"
    )
    ...
    ars["montant_seuil_non_versement"] = montant_seuil_non_versement = ars_min.pop(
        "montant_minimum_verse"
    )
    ars["plafond_ressources"] = plafond_ressources = ars_plaf.pop(
        "plafond_de_ressources_0_enfant"
    )
    ars["taux_primaire"] = taux_primaire = ars_m.pop(
        "enfants_entre_6_et_10_ans_en_de_la_bmaf_1"
    )
    ars["taux_college"] = taux_college = ars_m.pop(
        "enfants_entre_11_et_14_ans_en_de_la_bmaf_2"
    )
    ...
    cf[
        "majoration_plafond_biact_isole"
    ] = majoration_plafond_biact_isole = majoration.pop("biactifs_et_parents_isoles")
    cf[
        "majoration_plafond_2_premiers_enf"
    ] = majoration_plafond_2_premiers_enf = majoration.pop(
        "1er_et_2eme_enfants_en_du_plafond_de_ressources_avec_0_enfant"
    )
    cf[
        "majoration_plafond_3eme_enf_et_plus"
    ] = majoration_plafond_3eme_enf_et_plus = majoration.pop(
        "3eme_enfant_et_plus_en_du_plafond_de_ressources_avec_0_enfant"
    )
    complement_familial_en_de_la_bmaf_en_de_la_bmaf = cf[
        "complement_familial_en_de_la_bmaf_en_de_la_bmaf"
    ]
    cf[
        "taux_cf_base"
    ] = taux_cf_base = complement_familial_en_de_la_bmaf_en_de_la_bmaf.pop(
        "montant_de_base"
    )
    cf[
        "taux_cf_majore"
    ] = taux_cf_majore = complement_familial_en_de_la_bmaf_en_de_la_bmaf.pop(
        "montant_majore"
    )
    ...
    cf["taux_base_dom"] = taux_base_dom = complement_familial_en_de_la_bmaf_dom.pop(
        "montant_de_base"
    )
    cf["taux_majore_dom"] = taux_majore_dom = complement_familial_en_de_la_bmaf_dom.pop(
        "montant_majore"
    )
    cf["nombre_enfant_minimum_dom"] = nombre_enfant_minimum_dom = cf.pop(
        "nombre_d_enfant_minimum"
    )
    cf[
        "age_en_dessous_duquel_l_enfant_prive_la_famille_du_cf_dom"
    ] = age_en_dessous_duquel_l_enfant_prive_la_famille_du_cf_dom = cf.pop(
        "age_en_dessous_duquel_l_enfant_prive_la_famille_du_complement_familiale"
    )
    ...
    montant_mensuel_en_de_la_base_de_calcul = ape[
        "montant_mensuel_en_de_la_base_de_calcul"
    ]
    ape[
        "taux_activite_sup_50"
    ] = taux_activite_sup_50 = montant_mensuel_en_de_la_base_de_calcul.pop(
        "pour_une_activite_de_plus_de_85h_mois_50_duree_legale"
    )
    ape[
        "taux_activite_sup_80"
    ] = taux_activite_sup_80 = montant_mensuel_en_de_la_base_de_calcul.pop(
        "pour_une_activite_ou_formation_comprise_entre_50_et_80_de_la_duree_legale"
    )
    ape[
        "taux_inactivite"
    ] = taux_inactivite = montant_mensuel_en_de_la_base_de_calcul.pop("a_taux_plein")
    # apje
    ...
    apje["age_max_dernier_enf"] = age_max_dernier_enf = apje.pop(
        "age_limite_des_enfants_ouvrant_droit_a_l_apje_2"
    )
    ...
    apje[
        ...
        taux_enfant_1_et_2
    ) = majoration_en_ou_en_en_du_plafond_de_ressources_avec_0_enfant.pop(
        "1er_et_2eme_enfant"
    )
    apje[
        "taux_enfant_3_et_plus"
    ] = (
        taux_enfant_3_et_plus
    ) = majoration_en_ou_en_en_du_plafond_de_ressources_avec_0_enfant.pop(
        "3eme_enfant_et_plus"
    ...
    del prestations["paje_cm2"][
        "conditions_pour_qu_un_enfant_adopte_ouvre_droit_a_la_prime_a_son_arrivee"
    ]
    ...
    avant_2014["plafond_ressources_0_enf"] = paje_plaf.pop(
        "premier_plafond_ne_ou_adopte_avant_le_1er_avril_2014"
    ).pop("plafond_de_ressources_0_enfant")
    paje_plaf.update(
        paje_plaf.pop("majoration_en_ou_en_du_plafond_de_ressources_avec_0_enfant")
    )
    avant_2014["majoration_biact_parent_isoles"] = paje_plaf.pop(
        "biactifs_et_parents_isoles_1"
    )
    avant_2014["taux_majoration_2_premiers_enf"] = paje_plaf.pop("1er_et_2eme_enfant")
    avant_2014["taux_majoration_3eme_enf_et_plus"] = paje_plaf.pop(
        "3eme_enfant_et_plus"
    )
    paje["clmg"].update(
        clmg.pop("complement_libre_choix_du_mode_de_garde_en_de_la_bmaf_1")
    )
    clmg["taux_recours_emploi_1er_plafond"] = clmg.pop(
        "revenus_inferieurs_a_45_du_plafond_d_allocation"
    )
    clmg["taux_recours_emploi_2e_plafond"] = clmg.pop(
        "revenus_superieurs_a_45_du_plafond_d_allocation"
    )
    clmg["taux_recours_emploi_supp_2e_plafond"] = clmg.pop(
        "revenus_superieurs_au_plafond_d_allocation"
    )

    # plaf_cmg
    premier_plafond_ne_ou_adopte_avant_le_1er_avril_2014 = paje[
        "premier_plafond_ne_ou_adopte_avant_le_1er_avril_2014"
    ]
    deuxieme_plafond_ne_ou_adopte_avant_le_1er_avril_2014 = paje[
        "deuxieme_plafond_ne_ou_adopte_avant_le_1er_avril_2014"
    ]
    clmg["seuil11"] = premier_plafond_ne_ou_adopte_avant_le_1er_avril_2014.pop(
        "un_enfant"
    )
    clmg["seuil12"] = premier_plafond_ne_ou_adopte_avant_le_1er_avril_2014.pop(
        "deux_enfants"
    )
    clmg["seuil1sup"] = premier_plafond_ne_ou_adopte_avant_le_1er_avril_2014.pop(
        "majoration_pour_un_enfant_supplementaire"
    )
    clmg["seuil21"] = deuxieme_plafond_ne_ou_adopte_avant_le_1er_avril_2014.pop(
        "un_enfant"
    )
    clmg["seuil22"] = deuxieme_plafond_ne_ou_adopte_avant_le_1er_avril_2014.pop(
        "deux_enfants"
    )
    ...
    # cmg taux
    taux_pour_recours_a_une_assistante_maternelle_une_association_une_entreprise_ou_une_microcreche_en_de_la_bmaf = clmg[
        "taux_pour_recours_a_une_assistante_maternelle_une_association_une_entreprise_ou_une_microcreche_en_de_la_bmaf"
    ]
    ...
    clmg[
        "ass_mat1"
    ] = taux_pour_recours_a_une_assistante_maternelle_une_association_une_entreprise_ou_une_microcreche_en_de_la_bmaf.pop(
        "sous_le_premier_plafond"
    )
    clmg[
        "ass_mat2"
    ] = taux_pour_recours_a_une_assistante_maternelle_une_association_une_entreprise_ou_une_microcreche_en_de_la_bmaf.pop(
        "sous_le_second_plafond"
    )
    clmg[
        "ass_mat3"
    ] = taux_pour_recours_a_une_assistante_maternelle_une_association_une_entreprise_ou_une_microcreche_en_de_la_bmaf.pop(
        "apres_le_second_plafond"
    )
    clmg["domi1"] = taux_pour_recours_a_une_garde_a_domicile_en_de_la_bmaf.pop(
        "sous_le_premier_plafond"
    )
    clmg["domi2"] = taux_pour_recours_a_une_garde_a_domicile_en_de_la_bmaf.pop(
        "sous_le_second_plafond"
    )
    clmg["domi3"] = taux_pour_recours_a_une_garde_a_domicile_en_de_la_bmaf.pop(
        "apres_le_second_plafond"
    )
    # colca
    complement_optionnel_de_libre_choix_d_activite = paje.pop(
        "complement_optionnel_de_libre_choix_d_activite"
    )
    colca["avecab"] = complement_optionnel_de_libre_choix_d_activite.pop(
        "taux_plein_duree_de_l_arret_predeterminee"
    )
    ...
    apres_2014["plaf_tx_par_enf"] = paje.pop(
        "majoration_du_plafond_par_enfant_a_charge"
    )
    paje["prime_naissance"] = prime_naissance = dict()
    montant_en_de_la_bmaf = paje["montant_en_de_la_bmaf"]
    prime_naissance["prime_tx"] = montant_en_de_la_bmaf.pop(
        "prime_a_la_naissance_de_la_paje_1"
    )

    # autres
    def_pac = prestations["def_pac"]
    af["seuil_rev_taux"] = def_pac.pop(
        "revenu_plafond_pour_les_personnes_a_charge_n_etant_plus_sous_l_obligation_scolaire_en_du_smic_2"
    )
    bmaf = prestations["bmaf"]
    af["bmaf_ipp"] = bmaf_ipp = prestations.pop("bmaf")
    af["bmaf"] = bmaf_ipp.pop(
        "base_mensuelle_de_calcul_des_allocations_familiales_bmaf"
    )
    af["taux"] = taux = dict()
    ...
    asi["plafond_ressource_seul"] = plafond_ressource_seul = plafond_de_ressources.pop(
        "personnes_seules"
    )
    asi[
        "plafond_ressource_couple"
    ] = plafond_ressource_couple = plafond_de_ressources.pop("couples")
    ...
    asf["taux_1_parent"] = taux_1_parent = montant_de_l_asf_en_de_la_bmaf.pop(
        "orphelin_ou_assimile_d_un_seul_parent"
    )
    asf["taux_2_parents"] = taux_2_parents = montant_de_l_asf_en_de_la_bmaf.pop(
        "orphelin_ou_assimile_des_deux_parents"
    )
    ...
    rsa["duree_min_titre_sejour"] = duree_min_titre_sejour = rsa.pop(
        "duree_minimum_du_titre_de_sejour_annee"
    )
    rsa["majoration_parent_isole"] = majoration_parent_isole = rsa.pop(
        "majoration_pour_isolement_en_mois"
    )
    majoration_parent_isole = rsa["majoration_parent_isole"]
    majoration_parent_isole[
        "age_limite_enfant"
    ] = age_limite_enfant = majoration_parent_isole.pop(
        "age_limite_de_l_enfant_3_en_annee"
    )
    rsa["majoration_rsa"] = majoration_rsa = rsa.pop(
        "majoration_montant_maximal_en_de_la_base_rsa"
    )
    majoration_rsa[
        "taux_deuxieme_personne"
    ] = taux_deuxieme_personne = majoration_rsa.pop(
        "couples_ou_celibataire_avec_un_enfant"
    )
    majoration_rsa[
        "taux_troisieme_personne"
    ] = taux_troisieme_personne = majoration_rsa.pop(
        "couple_1_enfant_ou_pour_le_deuxieme_enfant_1"
    )
    majoration_rsa["taux_personne_supp"] = taux_personne_supp = majoration_rsa.pop(
        "par_enfant_supplementaire"
    )
    rsa["forfait_logement"] = forfait_logement = rsa.pop(
        "forfait_logement_en_du_montant_forfaitaire_du_rsa"
    )
    forfait_logement = rsa["forfait_logement"]
    forfait_logement["taux_1_personne"] = taux_1_personne = forfait_logement.pop(
        "1_personne"
    )
    forfait_logement["taux_2_personnes"] = taux_2_personnes = forfait_logement.pop(
        "2_personnes"
    )
    forfait_logement[
        "taux_3_personnes_ou_plus"
    ] = taux_3_personnes_ou_plus = forfait_logement.pop("3_personnes_ou_plus")
    ...
    ppa["pente"] = pente = ppa.pop(
        "majoration_des_ressources_sur_les_revenus_d_activite"
    )
    majoration_isolement_en_de_la_base_rsa = ppa[
        "majoration_isolement_en_de_la_base_rsa"
    ]
    ppa[
        "majoration_isolement_femme_enceinte"
    ] = (
        majoration_isolement_femme_enceinte
    ) = majoration_isolement_en_de_la_base_rsa.pop(
        "femmes_enceintes"
    )
    ppa[
        "majoration_isolement_enf_charge"
    ] = majoration_isolement_enf_charge = majoration_isolement_en_de_la_base_rsa.pop(
        "par_enfant_a_charge"
    )
    ...
    minima_sociaux["ppa"].update(
        ppa.pop("majoration_montant_maximal_en_de_la_base_rsa")
    )
    ppa["taux_deuxieme_personne"] = taux_deuxieme_personne = ppa.pop(
        "couples_ou_seul_avec_un_enfant"
    )
    ppa["taux_troisieme_personne"] = taux_troisieme_personne = ppa.pop(
        "couple_1_enfant_ou_pour_le_deuxieme_enfant_1"
    )
    ppa["taux_personne_supp"] = taux_personne_supp = ppa.pop(
        "par_enfant_supplementaire"
    )
    ...
    caah["majoration_vie_autonome"] = majoration_vie_autonome = caah.pop(
        "majoration_pour_la_vie_autonome"
    )
    ...
    taux_de_majoration_selon_la_taille_du_foyer_1 = aefa[
        "taux_de_majoration_selon_la_taille_du_foyer_1"
    ]
    ...
    aefa["tx_supp"] = taux_de_majoration_selon_la_taille_du_foyer_1.pop(
        "personne_supplementaire_si_conjoint"
    )
    ...
    root["taxation_capital"] = taxation_capital = root.pop(
        "baremes_ipp_taxation_capital"
    )
    taxation_capital["isf"] = isf = taxation_capital.pop("bareme_isf")
    seuils_des_tranches_du_bareme_de_l_isf = isf[
        "seuils_des_tranches_du_bareme_de_l_isf"
    ]
    taux_marginaux_des_tranches_du_bareme_de_l_isf = isf[
        "taux_marginaux_des_tranches_du_bareme_de_l_isf"
    ]
    ...
    decote["isf_base_decote"] = isf_base_decote = decote.pop(
        "parametre_de_calcul_de_la_decote_isf"
    )
    decote["isf_borne_sup_decote"] = isf_borne_sup_decote = decote.pop(
        "borne_superieure_de_la_decote"
    )
    decote["isf_borne_min_decote"] = isf_borne_min_decote = decote.pop(
        "borne_inferieure_de_la_decote"
    )
    ...
    isf["reduc_invest_don"] = reduc_invest_don = isf.pop(
        "reduction_pour_investissements_au_capital_de_pme"
    )
    ...
    reduc_invest_don[
        "taux_invest_direct_soc_holding"
    ] = taux_invest_direct_soc_holding = reduc_invest_don.pop(
        "taux_pour_investissement_direct_soc_holdings_fip_fcpi"
    )
    isf["reduc_invest_don"].update(
        isf.pop("reduction_pour_dons_a_certains_organismes_d_interet_general")
    )
    reduc_invest_don[
        "taux_don_interet_general"
    ] = taux_don_interet_general = reduc_invest_don.pop(
        "taux_pour_dons_a_certains_organismes_d_interet_general"
    )
    isf["reduc_pac"] = reduc_pac = dict()
    reduc_pac = isf["reduc_pac"]
    reduc_pac["reduc_enf_garde"] = reduc_enf_garde = isf.pop(
        "reduction_isf_enfant_a_charge"
    )
    isf["res_princ"] = res_princ = dict()
    res_princ = isf["res_princ"]
    res_princ[
        "abattement_sur_residence_principale"
    ] = abattement_sur_residence_principale = isf.pop(
        "abattement_sur_residence_principale"
    )
    #
    taxation_capital[
        "prelevements_sociaux"
    ] = prelevements_sociaux = taxation_capital.pop("ps")
    prelevements_sociaux["caps"] = caps = {}
    caps["produits_de_placement"] = prelevements_sociaux.pop(
        "caps_sur_les_produits_de_placement"
    )
    caps["revenus_du_patrimoine"] = prelevements_sociaux.pop(
        "caps_sur_les_revenus_du_patrimoine"
    )
