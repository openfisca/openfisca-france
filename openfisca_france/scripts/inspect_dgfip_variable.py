...
def inspect_dgfip_variable(variable, year, browser_name):
    ...
    url_base = (
        u"http://www3.finances.gouv.fr/calcul_impot/" + unicode(year + 1) + "/aides/"
    )
    url_section = {
    ...
    assert url_section is not None, "Unhandled section number: {}".format(
        section_number
    )
...
def main():
    ...
    parser.add_argument(
        "variable_name", help=u'Name of the variable to inspect. Example: "f7wr"'
    )
    parser.add_argument(
        "--browser",
        dest="browser_name",
        default="chromium",
        help=u"Open links in this browser",
    )
    parser.add_argument(
        "--min-year", default=None, help=u"Year to start from", type=int
    )
    ...
    parser.add_argument(
        "--year",
        default=None,
        help=u"Inspect the variable for the given year",
        type=int,
    )
