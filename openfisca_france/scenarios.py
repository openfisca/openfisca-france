...
class Scenario(scenarios.AbstractScenario):
    ...
    def post_process_test_case(self, test_case, period, state):
        ...
        individu_by_id = {
            individu["id"]: individu for individu in test_case["individus"]
        }
        ...
        test_case, error = conv.struct(
            dict(
                ...
                menages=conv.pipe(
                    conv.uniform_sequence(
                        conv.struct(
                            dict(personne_de_reference=conv.not_none), default=conv.noop
                        )
                    ),
                    conv.empty_to_none,
                    conv.not_none,
    ...
    def attribute_groupless_persons_to_entities(
        self, test_case, period, groupless_individus
    ):
        ...
        new_famille = dict(enfants=[], parents=[])
        new_famille_id = None
        for individu_id in individus_without_famille[:]:
            ...
            if (
                foyer_fiscal_role == u"declarants"
                and len(foyer_fiscal[u"declarants"]) == 2
            ):
                for declarant_id in foyer_fiscal[u"declarants"]:
                    if declarant_id != individu_id:
                        famille, other_role = find_famille_and_role(
                ...
                foyer_fiscal_role == u"personnes_a_charge"
                and foyer_fiscal[u"declarants"]
            ):
                for declarant_id in foyer_fiscal[u"declarants"]:
        ...
        new_foyer_fiscal = dict(declarants=[], personnes_a_charge=[])
        ...
        for individu_id in individus_without_foyer_fiscal[:]:
            ...
            if famille_role == u"parents" and len(famille[u"parents"]) == 2:
                for parent_id in famille[u"parents"]:
                    if parent_id != individu_id:
                        ...
                        if (
                            other_role == u"declarants"
                            and len(foyer_fiscal[u"declarants"]) == 1
                        ):
            ...
            if individu_id in individus_without_foyer_fiscal:
                ...
                if new_foyer_fiscal_id is None:
                    new_foyer_fiscal[u"id"] = new_foyer_fiscal_id = to_unicode(
                        uuid.uuid4()
                    )
        ...
        if famille and menage:
            ...
            if (
                not menage.get("personne_de_reference")
                and parent_1 in individus_without_menage
            ):
                menage["personne_de_reference"] = parent_1
                individus_without_menage.remove(parent_1)

        new_menage = dict(
            autres=[], conjoint=None, enfants=[], personne_de_reference=None
        )
        new_menage_id = None
            ...
            if famille_role == u"parents" and len(famille[u"parents"]) == 2:
                for parent_id in famille[u"parents"]:
                    if parent_id != individu_id:
                        ...
                        if (
                            other_role == u"personne_de_reference"
                            and menage[u"conjoint"] is None
                        ):
                            ...
                            other_role == u"conjoint"
                            and menage[u"personne_de_reference"] is None
                        ):
                            # Quand l'individu n'est pas encore dans un ménage, mais qu'il est parent
                            # dans une famille, qu'il y a un autre parent dans cette famille et que cet autre
    ...
    def suggest(self):
        ...
        for individu in test_case["individus"]:
            ...
            if (
                individu.get("age") is None
                and individu.get("age_en_mois") is None
                and individu.get("date_naissance") is None
            ):
                # Add missing date_naissance date to person (a parent is 40 years old and a child is 10 years old.
                is_parent = any(
                    individu_id in famille["parents"]
                    for famille in test_case["familles"]
                )
                birth_year = (
                    period_start_year - 40 if is_parent else period_start_year - 10
                )
                date_naissance = datetime.date(birth_year, 1, 1)
                individu["date_naissance"] = date_naissance
                suggestions.setdefault("test_case", {}).setdefault(
                    "individus", {}
                ).setdefault(individu_id, {})[
                    "date_naissance"
                ] = date_naissance.isoformat()
            if individu.get("activite") is None:
                if find_age(individu, period_start_date) < 16:
                    individu["activite"] = TypesActivite.etudiant
                    suggestions.setdefault("test_case", {}).setdefault(
                        "individus", {}
                    ).setdefault(individu_id, {})["activite"] = TypesActivite.etudiant
        ...
        for foyer_fiscal in test_case["foyers_fiscaux"]:
            if (
                len(foyer_fiscal["declarants"]) == 1
                and foyer_fiscal["personnes_a_charge"]
            ):
                ...
                if foyer_fiscal.get("caseT") is None:
                    suggestions.setdefault("test_case", {}).setdefault(
                        "foyers_fiscaux", {}
                    ).setdefault(foyer_fiscal["id"], {})["caseT"] = foyer_fiscal[
                        "caseT"
                    ] = True
                ...
                for individu_id in foyer_fiscal["declarants"]:
                    individu = individu_by_id[individu_id]
                    if (
                        individu.get("statut_marital") == TypesStatutMarital.marie
                    ):  # Marié
                ...
                for individu_id in foyer_fiscal["declarants"]:
                    ...
                    if individu.get("statut_marital") is None:
                        ...
                        suggestions.setdefault("test_case", {}).setdefault(
                            "individus", {}
                        ).setdefault(individu_id, {})["statut_marital"] = to_unicode(
                            statut_marital
                        )
...
def find_age(individu, date, default=None):
    ...
    if date_naissance is not None:
        ...
        if (
            date.month < date_naissance.month
            or date.month == date_naissance.month
            and date.day < date_naissance.day
        ):
            age -= 1
        return age
