...
class covid_aide_exceptionnelle_tpe_eligible(Variable):
    ...
    def formula_2020_03(individu, period):
        ...
        chiffre_d_affaire_annee_mois_un_an_avant = individu(
            "rpns_auto_entrepreneur_chiffre_affaires", period_1
        )
        chiffre_d_affaire_annee_moyenne_annee_precedente = (
            individu(
        ...
        chiffre_d_affaire_annee_n_1 = max_(
            chiffre_d_affaire_annee_mois_un_an_avant,
            chiffre_d_affaire_annee_moyenne_annee_precedente
        )
        return individu("travailleur_non_salarie", period) * (
                (
                        (chiffre_d_affaire - chiffre_d_affaire_annee_n_1)
                        / chiffre_d_affaire_annee_n_1
                ) < -0.5
        )
...
class covid_activite_partielle_montant(Variable):
    ...
    def formula_2020_03(individu, period, parameters):
        ...
        """
        eligibilite_activite_partielle = individu(
            "covid_activite_partielle_eligible", period
        )
        ...
        indemnite_horaire = max(
            indemnite_ap.taux * salaire_horaire, indemnite_ap.plancher
        )
