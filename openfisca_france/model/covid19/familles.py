...
class covid_aide_exceptionnelle_famille_montant(Variable):
    ...
    def formula_2020_05(famille, period, parameters):
        ...
        prim_forf = (
                famille.sum(
                    famille.members("prime_forfaitaire_mensuelle_reprise_activite", period)
                )
                > 0
        )
        age_i = famille.members("age", period)
        etudiant_i = famille.members("etudiant", period)
        moins_de_25_ans_non_etudiant = famille.any(
            (age_i <= 24) * (etudiant_i == 0), role=Famille.PARENT
        )
        ...
        moins_de_25_ans_non_etudiant_n_1 = famille.any(
            (age_i_n_1 <= 24) * (etudiant_i_n_1 == 0), role=Famille.PARENT
        )
        ...
        montant_n_1 = (
            ...
            + base_n_1
            * not_(base_jeune_n_1)
            * (montants.base + montants.par_enfant * af_nbenf_n_1)
            + not_(base_n_1)
            * not_(base_jeune_n_1)
            * al_n_1 * af_nbenf_n_1
            * montants.par_enfant
