...
class aide_jeunes_diplomes_anciens_boursiers_base_ressources(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        nombre_mensualites = parameters(
            period
        ).bourses_enseignement_superieur.criteres_sociaux.nombre_mensualites
...
class aide_jeunes_diplomes_anciens_boursiers_eligibilite(Variable):
    ...
    def formula_2021_02_05(individu, period, parameters):
        age_limite = parameters(
            period
        ).covid19.aide_jeunes_diplomes_anciens_boursiers.age_limite
        ...
        condition_bourse = (
                                   individu("aide_jeunes_diplomes_anciens_boursiers_base_ressources", period)
                                   > 0
        )

        # ne pas être en formation (de niveau 5 minimum) au moment de la demande.
        pas_en_formation = (
                individu("niveau_diplome_formation", period)
                == TypesNiveauDiplome.non_renseigne
        )
...
class aide_jeunes_diplomes_anciens_boursiers_montant(Variable):
    ...
    def formula_2021_02_05(individu, period, parameters):
        aide_jeunes_diplomes_anciens_boursiers_eligibilite = individu(
            "aide_jeunes_diplomes_anciens_boursiers_eligibilite", period
        )

        bourse_precedente = individu(
            "aide_jeunes_diplomes_anciens_boursiers_base_ressources", period
        )
        parameters_aide = parameters(
            period
        ).covid19.aide_jeunes_diplomes_anciens_boursiers

        part_bourse = bourse_precedente * parameters_aide.taux_bourse

        statut_occupation_logement = individu.menage(
        ...
        condition_logement = not_(
            ...
            + (
                    statut_occupation_logement
                    == TypesStatutOccupationLogement.non_renseigne
            )
        )
        part_logement = condition_logement * parameters_aide.majoration_logement

        return aide_jeunes_diplomes_anciens_boursiers_eligibilite * (
                part_bourse + part_logement
        )
