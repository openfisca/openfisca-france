...
class indemnite_accident_travail(Variable):
    ...
    def formula(individu, period, parameters):
        indem_at = parameters(period).prestations_sociales.solidarite_insertion.accident_travail.rente.taux
        taux_incapacite = individu('taux_accident_travail', period)
...
class rente_accident_travail_base(Variable):
    ...
    def formula(individu, period, parameters):
        param_rente_at = parameters(period).prestations_sociales.solidarite_insertion.accident_travail.rente.taux
        taux_incapacite = individu('taux_accident_travail', period)
...
class rente_accident_travail_apres_rachat(Variable):
    ...
    def formula(individu, period, parameters):
        rente_at = parameters(period).prestations_sociales.solidarite_insertion.accident_travail.rente.taux
...
class rente_accident_travail_salaire_utile(Variable):
    ...
    def formula(individu, period, parameters):
        previous_year = period.start.period('year').offset(-1)
        rente_at = parameters(period).prestations_sociales.solidarite_insertion.accident_travail.rente
