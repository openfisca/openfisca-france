...
class enfant_a_charge(Variable):
    value_type = bool
    entity = Individu
    label = u"Enfant à charge non marié, de moins de 18 ans au 1er janvier de l'année de perception des" u" revenus, ou né durant la même année, ou handicapés quel que soit son âge"
    definition_period = YEAR
...
class nbF(Variable):
    ...
    entity = FoyerFiscal
    value_type = float
    label = u"Nombre d'enfants à charge non mariés, qui ne sont pas en résidence alternée, de moins de 18 ans au 1er janvier de l'année de perception des" u" revenus, ou nés durant la même année ou handicapés quel que soit leur âge"
    definition_period = YEAR
...
class nbJ(Variable):
    ...
    def formula(foyer_fiscal, period):
        enfant_majeur_celibataire_sans_enfant = foyer_fiscal.members(
            "enfant_majeur_celibataire_sans_enfant", period
        )
...
class nombre_enfants_majeurs_celibataires_sans_enfant(Variable):
    ...
    def formula(menage, period):
        enfant_majeur_celibataire_sans_enfant = menage.members(
            "enfant_majeur_celibataire_sans_enfant", period
        )
...
class revenu_assimile_salaire_apres_abattements(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        abatfor = round_(
            min_(
                max_(abatpro.taux * revenu_assimile_salaire, abattement_minimum),
                abatpro.max,
            )
        )
        return (frais_reels > abatfor) * (revenu_assimile_salaire - frais_reels) + (
            frais_reels <= abatfor
...
class revenu_assimile_pension(Variable):
    ...
    def formula(individu, period):
        ...
        pensions_alimentaires_percues_decl = individu(
            "pensions_alimentaires_percues_decl", period, options=[ADD]
        )
        ...
        return (
            pensions_alimentaires_percues * pensions_alimentaires_percues_decl
            + retraite_imposable
            + pension_invalidite
        )
...
class revenu_assimile_pension_apres_abattements(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        return max_(
            ...
            - round_(max_(abatpen.taux * revenu_assimile_pension, abatpen.min)),
        )
...
class indu_plaf_abat_pen(Variable):
    ...
    def formula(foyer_fiscal, period, parameters):
        ...
        pen_net_i = foyer_fiscal.members(
            "revenu_assimile_pension_apres_abattements", period
        )
        abatpen = parameters(period).impot_revenu.tspr.abatpen
...
class abattement_salaires_pensions(Variable):
    ...
    def formula(individu, period, parameters):
        revenu_assimile_salaire_apres_abattements = individu(
            "revenu_assimile_salaire_apres_abattements", period
        )
        revenu_assimile_pension_apres_abattements = individu(
            "revenu_assimile_pension_apres_abattements", period
        )
...
class traitements_salaires_pensions_rentes(Variable):
    ...
    def formula(individu, period):
        ...
        # correspondante.
        rente_viagere_titre_onereux_net = individu.foyer_fiscal(
            "rente_viagere_titre_onereux_net", period.offset("first-of")
        )
...
class rev_cat_rvcm(Variable):
    ...
    def formula_2002_01_01(foyer_fiscal, period, parameters):
        ...
        TOT3 = (
            (f2ts - F2)
            + f2go * rvcm.majoration_revenus_reputes_distribues
            + f2tr_bis
            - g12a
        )
    ...
    def formula_2013_01_01(foyer_fiscal, period, parameters):
        ...
        rvcm_apres_abattement = (
            f2fu
            + f2dc
            - abattement_dividende
            + f2ch
            - min_(f2ch, abattement_assurance_vie)
            + f2ts
            + f2tr
...
class rev_cat_rfon(Variable):
    ...
    def formula(foyer_fiscal, period, parameters):
        ...
        if ((f4be != 0) & ((f4ba != 0) | (f4bb != 0) | (f4bc != 0))).any():
            log.error(
                (
                    "Problème de déclarations des revenus : incompatibilité de la déclaration des revenus fonciers (f4ba, f4bb, f4bc) et microfonciers (f4be)"
                )
            )
...
class rbg(Variable):
    ...
    def formula(foyer_fiscal, period, parameters):
        ...
        return max_(
            ...
            rev_cat
            + f6gh
            + (foyer_fiscal.sum(nbic_impm_i) + nacc_pvce) * (1 + cga)
            - deficit_ante,
        )
...
class csg_deduc_patrimoine_simulated(Variable):
    ...
    def formula(foyer_fiscal, period, parameters):
        ...
        """
        rev_cat_rfon = foyer_fiscal("rev_cat_rfon", period)
        revenus_capitaux_prelevement_bareme = foyer_fiscal(
            "revenus_capitaux_prelevement_bareme", period
        )
        rente_viagere_titre_onereux = foyer_fiscal(
            "rente_viagere_titre_onereux", period
        )
...
class ir_brut(Variable):
    ...
    entity = FoyerFiscal
    label = (
        u"Impôt sur le revenu brut avant non imposabilité et plafonnement du quotient"
    )
    definition_period = YEAR
    ...
    def formula(foyer_fiscal, period, parameters):
        ...
        return (taux_effectif == 0) * nbptr * bareme.calc(
            rni / nbptr
        ) + taux_effectif * rni
...
class ir_plaf_qf(Variable):
    ...
    def formula(foyer_fiscal, period, parameters):
        ...
        condition63 = (
            (celibataire_ou_divorce | (veuf & not_(jeune_veuf)))
            & not_(caseN)
            & (nb_pac == 0)
            & (caseK | caseE)
            & (caseH < int(period.start.year) - 25)
        )
        ...
        condition62caa = condition62caa0 & (
            condition62caa1 | condition62caa2 | condition62caa3
        )
        # marié pacs
        ...
        D = plafond_qf.reduc_postplafond * (
            condition62ca
            + ~condition62ca
            * condition62cb
    ...
    def formula_2013_01_01(foyer_fiscal, period, parameters):
        ...
        condition62c = (
            caseP | caseF | caseW | caseS | caseG | (nbG > 0) | (nbI > 0) | nbR > 0
        )
        ...
        conditionGuadMarReu = (
            0
        )  # faire une condition avec département + code commune (Depcom qui est pour l'instant une variable ménage)
        conditionGuyMay = 0  # provisoire
        conditionDOM = conditionGuadMarReu | conditionGuyMay

        abat_dom = conditionGuadMarReu * min_(
            plafond_qf.abat_dom.plaf_GuadMarReu,
            plafond_qf.abat_dom.taux_GuadMarReu * IP1,
        ) + conditionGuyMay * min_(
            plafond_qf.abat_dom.plaf_GuyMay, plafond_qf.abat_dom.taux_GuyMay * IP1
        )
        IP2 = IP1 - abat_dom
...
class decote(Variable):
    ...
    def formula_2015_01_01(foyer_fiscal, period, parameters):
        ...
        decote_celib = (ir_plaf_qf < 4 / 3 * decote_seuil_celib) * (
            decote_seuil_celib - 3 / 4 * ir_plaf_qf
        )
        decote_couple = (ir_plaf_qf < 4 / 3 * decote_seuil_couple) * (
            decote_seuil_couple - 3 / 4 * ir_plaf_qf
        )
...
class decote_gain_fiscal(Variable):
    ...
    entity = FoyerFiscal
    label = (
        u"Gain fiscal de la décote/Décote au sens Dgfip tel que sur la feuille d'impôt"
    )
    definition_period = YEAR
...
class reduction_ss_condition_revenus(Variable):
    ...
    def formula_2016_01_01(foyer_fiscal, period, parameters):
        ...
        reduction2 = (P.taux * ir_apres_plaf_qf_et_decote * (plafond2 - rfr)) / (
            (plafond2 - plafond1) * nb_adult
        )

        reduction_sous_condition_de_ressources = (rfr < plafond1) * reduction1 + (
            rfr >= plafond1
        ) * (rfr < plafond2) * reduction2
...
class ip_net(Variable):
    ...
    def formula_2016_01_01(foyer_fiscal, period, parameters):
        ...
        decote = foyer_fiscal("decote", period)
        ir_plaf_qf = foyer_fiscal("ir_plaf_qf", period)
        reduction_ss_condition_revenus = foyer_fiscal(
            "reduction_ss_condition_revenus", period
        )
        taux = parameters(period).impot_revenu.rpns.taux16
        ...
        return max_(
            0,
            ir_plaf_qf
            + foyer_fiscal.sum(cncn_info_i) * taux
            - decote
...
class rfr(Variable):
    ...
    def formula(foyer_fiscal, period, parameters):
        ...
        """
        abattement_net_retraite_dirigeant_pme = foyer_fiscal(
            "abattement_net_retraite_dirigeant_pme", period
        )
        abattement_net_duree_detention = foyer_fiscal(
            "abattement_net_duree_detention", period
        )
        f2dm = foyer_fiscal("f2dm", period)
        microentreprise = foyer_fiscal("microentreprise", period)
        revenus_capitaux_prelevement_liberatoire = foyer_fiscal(
            "revenus_capitaux_prelevement_liberatoire", period, options=[ADD]
        )
        ...
        return (
            max_(0, rni)
            + rfr_charges_deductibles
            + rfr_plus_values
            + rfr_rev_capitaux_mobiliers
...
class imp_lib(Variable):
    ...
    def formula_2002_01_01(foyer_fiscal, period, parameters):
        ...
        prelevement_liberatoire = parameters(
            period
        ).impot_revenu.rvcm.prelevement_liberatoire

        return -(
            prelevement_liberatoire.assvie * f2dh + prelevement_liberatoire.autre * f2ee
        )
    ...
    def formula_2008_01_01(foyer_fiscal, period, parameters):
        ...
        return (
            -(
                prelevement_liberatoire.action * f2da
                + prelevement_liberatoire.autre * f2ee
            )
...
class defrag(Variable):
    ...
    def formula(foyer_fiscal, period, parameters):
        ...
        return min_(
            ...
            (1 + cga) * (frag_impo + nrag_impg + frag_pvct) + arag_impg + frag_fore,
        )
...
class defacc(Variable):
    ...
    entity = FoyerFiscal
    label = (
        u"Déficit industriels et commerciaux non professionnels des années antérieures"
    )
    definition_period = YEAR
    ...
    def formula(foyer_fiscal, period, parameters):
        ...
        macc_timp = abat_rpns(macc_impv, micro.specialbnc.marchandises) + abat_rpns(
            macc_imps, micro.specialbnc.services
        )
        return min_(
            f5rn + f5ro + f5rp + f5rq + f5rr + f5rw,
            aacc_impn + macc_pvct + macc_timp + (1 + cga) * nacc_impn,
        )
...
class defncn(Variable):
    ...
    def formula(foyer_fiscal, period, parameters):
        ...
        return min_(
            ...
            abat_rpns(mncn_impo, specialbnc.services)
            + mncn_pvct
            + cncn_aimp
            + (1 + cga) * cncn_bene,
        )  #  TODO check !
...
class defmeu(Variable):
    ...
    entity = FoyerFiscal
    label = (
        u"Déficit des locations meublées non professionnelles des années antérieures"
    )
    definition_period = YEAR
...
class ric(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        taux = (
            micro.specialbnc.marchandises.taux * cond
            + micro.specialbnc.services.taux * not_(cond)
        )
        ...
        cbic = min_(
            ...
            max_(
                ...
                round_(
                    mbic_impv * micro.specialbnc.marchandises.taux
                    + mbic_imps * micro.specialbnc.services.taux
                    + mbic_exon * taux
                ),
            ),
        )
...
class rac(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        cacc = min_(
            macc_impv + macc_imps + macc_exon + mncn_impo,
            max_(
                micro.specialbnc.marchandises.min,
                round_(
                    macc_impv * micro.specialbnc.marchandises.taux
                    + macc_imps * micro.specialbnc.services.taux
                    + macc_exon * taux
...
class moins_values_court_terme_nonpro(Variable):
    ...
    def formula(individu, period, parameters):
        macc_mvct = individu.foyer_fiscal("macc_mvct", period) * individu.has_role(
            FoyerFiscal.DECLARANT_PRINCIPAL
        )
        mncn_mvct = individu.foyer_fiscal("mncn_mvct", period) * individu.has_role(
            FoyerFiscal.DECLARANT_PRINCIPAL
        )
...
class moins_values_court_terme_pro(Variable):
    ...
    def formula(individu, period, parameters):
        mbic_mvct = individu.foyer_fiscal("mbic_mvct", period) * individu.has_role(
            FoyerFiscal.DECLARANT_PRINCIPAL
        )
...
class moins_values_court_terme_non_salaries(Variable):
    ...
    entity = Individu
    label = (
        u"Moins values de court terme - Revenu des professions non salariées (toutes)"
    )
    definition_period = YEAR
...
class rpns_revenus_forfait_agricole(Variable):
    ...
    definition_period = YEAR
        ...
        "2015-12-31"
    )  # TODO : le forfait agricole est remplaçé par le régime micro-bénéfices agricoles à partir de 2016
...
class rpns_individu(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        mbic_timp = abat_rpns(mbic_impv, micro.specialbnc.marchandises) + abat_rpns(
            mbic_imps, micro.specialbnc.services
        )
        ...
        majo_cga = max_(
            0, cga_taux2 * (ntimp + rpns_frag)
        )  # Pour ne pas avoir à majorer les déficits
        # total 6
        revevenus_non_salaries = (
            rpns_frag + frag_fore + atimp + ntimp + majo_cga - def_agri
        )
        ...
        exon_acc = (
            max_(0, macc_timp + nacc_timp - macc_mvct) - macc_timp - nacc_timp
        )  # ajout artificiel
...
class abat_spe(Variable):
    ...
    def formula(foyer_fiscal, period, parameters):
        ...
        abattements_personne_agee_ou_invalide = (
            abattements_rni.personne_agee_ou_invalide
        )
        ...
        nb_elig_as = 1 * (((ageV >= 65) | invV) & (ageV > 0)) + 1 * (
            ((ageC >= 65) | invC) & (ageC > 0)
        )
        as_inv = (
            nb_elig_as
            * abattements_personne_agee_ou_invalide.montant
            * (
                (rng <= abattements_personne_agee_ou_invalide.plafond_de_ressources_1)
                + (
                    (
                        rng
                        > abattements_personne_agee_ou_invalide.plafond_de_ressources_1
                    )
                    & (
                        rng
                        <= abattements_personne_agee_ou_invalide.plafond_de_ressources_2
                    )
                )
                * 0.5
            )
...
class taux_effectif(Variable):
    ...
    def formula_2009_01_01(foyer_fiscal, period, parameters):
        ...
        return (
            trigger * nbptr * bareme.calc(base_fictive / nbptr) / max_(1, base_fictive)
        )
...
class nbptr(Variable):
    ...
    def formula(foyer_fiscal, period, parameters):
        ...
        enf1 = (no_pac & has_alt) * (
            quotient_familial.enf1 * min_(nbH, 2) * 0.5
...
class ppe_elig(Variable):
    ...
    def formula(foyer_fiscal, period, parameters):
        ...
        seuil = (veuf | celibataire_ou_divorce) * (
            ppe.eligi1 + 2 * max_(nbptr - 1, 0) * ppe.eligi3
...
class ppe_brute(Variable):
    ...
    def formula(foyer_fiscal, period, parameters):
        ...
        nb_pac_ppe = max_(
            0, nb_pac - foyer_fiscal.sum(eligible_i, role=FoyerFiscal.PERSONNE_A_CHARGE)
        )
        ...
        def ppe_bar2(base):
            return (base <= ppe.seuil2) * (base) * ppe.taux1 + (
                (base > ppe.seuil2) & (base <= ppe.seuil3)
            ) * (ppe.seuil3 - base) * ppe.taux2
        ...
        ppe_monact_vous = (
            eliv & ligne2 & (basevi >= ppe.seuil1) & (basev <= ppe.seuil4)
        ) * ppe.monact
        ppe_monact_conj = (
            elic & ligne2 & (baseci >= ppe.seuil1) & (basec <= ppe.seuil4)
        ) * ppe.monact
        ...
        maj_pac = (
            ...
            * (
                ...
                * ppe.pac
                * (nb_pac_ppe + nbH * 0.5)
                + (ligne2 & (base_monacti >= ppe.seuil1) & (base_monact <= ppe.seuil3))
                * ppe.pac
                * (nb_pac_ppe + nbH * 0.5)
                + (ligne2 & (base_monact > ppe.seuil3) & (base_monact <= ppe.seuil5))
                * ppe.pac
                * ((nb_pac_ppe != 0) + 0.5 * ((nb_pac_ppe == 0) & (nbH != 0)))
                + (ligne3 & (basevi >= ppe.seuil1) & (basev <= ppe.seuil3))
                * (
                    (
                        min_(nb_pac_ppe, 1) * 2 * ppe.pac
                        + max_(nb_pac_ppe - 1, 0) * ppe.pac
                    )
                    + (nb_pac_ppe == 0)
                    * (min_(nbH, 2) * ppe.pac + max_(nbH - 2, 0) * ppe.pac * 0.5)
                )
                + (ligne3 & (basev > ppe.seuil3) & (basev <= ppe.seuil5))
                * ppe.pac
        ...
        def coef(coef_tp):
            return (coef_tp <= 0.5) * coef_tp * 1.45 + (coef_tp > 0.5) * (
                0.55 * coef_tp + 0.45
            )
