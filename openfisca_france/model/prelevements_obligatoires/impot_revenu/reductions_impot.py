...
class reductions(Variable):
    ...
    def formula_2013_01_01(foyer_fiscal, period, parameters):
        ...
        patnat = foyer_fiscal("patnat", period)
        prcomp = foyer_fiscal("prcomp", period)
        reduction_impot_exceptionnelle = foyer_fiscal(
            "reduction_impot_exceptionnelle", period
        )
        repsoc = foyer_fiscal("repsoc", period)
        resimm = foyer_fiscal("resimm", period)
        rsceha = foyer_fiscal("rsceha", period)
...
class cappme(Variable):
    ...
    def formula_2009_01_01(foyer_fiscal, period, parameters):
        ...
        seuil = P.seuil * (maries_ou_pacses + 1)
        seuil = P.seuil_tpe * (maries_ou_pacses + 1) * (f7cu > 0) + P.seuil * (
            maries_ou_pacses + 1
        ) * (f7cu <= 0)
        return P.taux * min_(base, seuil)
    ...
    def formula_2012_01_01(foyer_fiscal, period, parameters):
        ...
        seuil2 = max_(
            0,
            P.seuil_tpe * (maries_ou_pacses + 1)
            - min_(base, seuil1)
            - min_(f7cq, seuil1)
            - min_(f7cu, seuil1),
        )
    ...
    def formula_2014_01_01(foyer_fiscal, period, parameters):
        ...
        base_report_cappme_2011_PME = max_(
            0, min_(f7cm, seuil1) - base_report_cappme_2010_PME
        )
        base_report_cappme_2012_PME = max_(
            0,
            min_(
                f7cn, seuil1 - base_report_cappme_2010_PME - base_report_cappme_2011_PME
            ),
        )
        base_report_cappme_2013_PME = max_(
            0,
            min_(
                f7cc,
                seuil1
                - base_report_cappme_2010_PME
                - base_report_cappme_2011_PME
                - base_report_cappme_2012_PME,
            ),
        )
        base_cappme_2014_PME = max_(
            0,
            min_(
                f7cu,
                seuil1
        ...
        base_report_cappme_2013_TPE = max_(
            0, min_(f7cr, seuil2 - base_report_cappme_2012_TPE)
        )
        base_cappme_2014_TPE = max_(
            0,
            min_(
                f7cf, seuil2 - base_report_cappme_2012_TPE - base_report_cappme_2013_TPE
            ),
        )

        seuil3 = seuil2 - min_(seuil2, base_report_cappme_2010_PME)
        seuil4 = seuil3 - min_(
            seuil3, base_report_cappme_2010_PME + base_report_cappme_2011_PME
        )
    ...
    def formula_2015_01_01(foyer_fiscal, period, parameters):
        ...
        base_report_cappme_2012_PME = max_(
            0, min_(f7cm, seuil1) - base_report_cappme_2011_PME
        )
        base_report_cappme_2013_PME = max_(
            0,
            min_(
                f7cn, seuil1 - base_report_cappme_2011_PME - base_report_cappme_2012_PME
            ),
        )
        base_report_cappme_2014_PME = max_(
            0,
            min_(
                f7cc,
                seuil1
        ...
        base_report_cappme_2014_TPE = max_(
            0,
            min_(
                f7cv, seuil2 - base_report_cappme_2012_TPE - base_report_cappme_2013_TPE
            ),
        )
        base_cappme_2015_TPE = max_(
            0,
            min_(
                f7cf,
                seuil2
    ...
    def formula_2016_01_01(foyer_fiscal, period, parameters):
        ...
        base_report_cappme_2013_PME = max_(
            0, min_(f7cm, seuil1) - base_report_cappme_2012_PME
        )
        base_report_cappme_2014_PME = max_(
            0,
            min_(
                f7cn, seuil1 - base_report_cappme_2012_PME - base_report_cappme_2013_PME
            ),
        )
        ...
        base_report_cappme_2015_TPE = max_(
            0,
            min_(
                f7cx,
                seuil2
    ...
    def formula_2017_01_01(foyer_fiscal, period, parameters):
        ...
        base_report_cappme_2014_PME = max_(
            0, min_(f7cm, plafond_PME - base_report_cappme_2013_PME)
        )
        base_report_cappme_2015_PME = max_(
            0,
            min_(
                f7cn,
                plafond_PME - base_report_cappme_2013_PME - base_report_cappme_2014_PME,
            ),
        )
        base_report_cappme_2016_PME = max_(
            0,
            min_(
                f7cc,
                plafond_PME
        ...
        base_report_cappme_2014_TPE = max_(
            0, min_(f7cr, plafond_TPE - base_report_cappme_2013_TPE)
        )
        base_report_cappme_2015_TPE = max_(
            0,
            min_(
                f7cv,
                plafond_TPE - base_report_cappme_2013_TPE - base_report_cappme_2014_TPE,
            ),
        )
        base_report_cappme_2016_TPE = max_(
            0,
            min_(
                f7cx,
                plafond_TPE
...
class cotsyn(Variable):
    ...
    def formula(foyer_fiscal, period, parameters):
        ...
        salaire_imposable = foyer_fiscal.members(
            "salaire_imposable", period, options=[ADD]
        )
        chomage_imposable = foyer_fiscal.members(
            "chomage_imposable", period, options=[ADD]
        )
        retraite_imposable = foyer_fiscal.members(
...
class creaen(Variable):
    ...
    def formula_2009_01_01(foyer_fiscal, period, parameters):
        ...
        return P.base * ((f7jy + f7fy) + f7hy / 2) + P.hand * ((f7ky + f7gy) + f7iy / 2)
    ...
    def formula_2010_01_01(foyer_fiscal, period, parameters):
        ...
        return P.base * ((f7jy + f7fy) + (f7hy + f7ly) / 2) + P.hand * (
            (f7ky + f7gy) + (f7iy + f7my) / 2
        )
    ...
    def formula_2012_01_01(foyer_fiscal, period, parameters):
        ...
        return P.base * (f7ly / 2) + P.hand * (f7my / 2)
...
class dfppce(Variable):
    ...
    def formula_2013_01_01(foyer_fiscal, period, parameters):
        ...
        f7vc = foyer_fiscal("f7vc", period)
        P = parameters(period).impot_revenu.reductions_impots.dons
        plafond_reduction_donapd = parameters(
            period
        ).impot_revenu.reductions_impots.donapd.max

        report_f7va = max_(0, f7va - plafond_reduction_donapd)
        base = (
            min_(P.max_niv, f7uf + f7uh)
            + f7vc
            + f7xs
            + f7xt
            + f7xu
...
class doment(Variable):
    ...
    def formula_2009_01_01(foyer_fiscal, period, parameters):
        ...
        return (
            f7oz + f7pz + f7qz + f7rz + f7sz + f7qe + f7qf + f7qg + f7qh + f7qi + f7qj
        )
...
class duflot(Variable):
    ...
    def formula_2013_01_01(foyer_fiscal, period, parameters):
        ...
        return (
            ...
            + min_(P.plafond, invest_domtom_2013) * P.taux_om
        ) / 9
    ...
    def formula_2014_01_01(foyer_fiscal, period, parameters):
        ...
        max1 = max_(
            0, P.plafond - invest_domtom_2014 - f7qd
        )  # 2014 : plafond commun 'duflot' et 'rpinel'
...
class ecodev(Variable):
    ...
    def formula_2009_01_01(foyer_fiscal, period, parameters):
        ...
        return min_(
            f7uh * P.taux, min_(P.taux_plafond * rbg_int, P.plafond_par_personne)
        )  # page3 ligne 18
...
class garext(Variable):
    ...
    entity = FoyerFiscal
    label = (
        u"Réduction d'impôt en faveur des dépenses de frais de garde des jeunes enfants"
    )
...
class invlst(Variable):
    ...
    entity = FoyerFiscal
    label = (
        u"Réduction d'impôt en faveur des investissements dans le secteur touristique"
    )
    ...
    def formula_2014_01_01(foyer_fiscal, period, parameters):
        ...
        reduction_residence_sociale = P.taux_xo * (
            report_residence_sociale_2009 + report_residence_sociale_2010
        )

        return around(
            reduction_logement_neuf
            + reduction_rehabilitation
            + reduction_residence_sociale
        )
    ...
    def formula_2017_01_01(foyer_fiscal, period, parameters):
        ...
        reduction_logement_neuf = P.taux_xi * (
            report_logement_neuf_2011 + report_logement_neuf_2012
        )
        reduction_rehabilitation = P.taux_xj * (
            report_rehabilitation_2011 + report_rehabilitation_2012
        )
...
class locmeu(Variable):
    ...
    def formula_2010_01_01(foyer_fiscal, period, parameters):
        ...
        return (
            (min_(P.max, max_(f7ij, f7il)) + min_(P.max, f7im)) / 9 + f7ik
        ) * P.taux + f7is
    ...
    def formula_2011_01_01(foyer_fiscal, period, parameters):
        ...
        taux_reduc_2011 = P.taux20 * majoration_taux_invest_2011 + P.taux18 * not_(
            majoration_taux_invest_2011
        )
    ...
    def formula_2012_01_01(foyer_fiscal, period, parameters):
        ...
        taux_reduc_2012 = P.taux18 * majoration_taux_invest_2012 + P.taux11 * not_(
            majoration_taux_invest_2012
        )
        report = (
    ...
    def formula_2014_01_01(foyer_fiscal, period, parameters):
        ...
        return (
            (
                (min_(P.max, max_(f7im, f7iw)) + min_(P.max, f7io))
                * taux_reduc_2009_2010
                + min_(P.max, maxi(f7ij, f7il, f7in, f7iv)) * taux_reduc_2011
                + min_(  # to check : impossible de remplir à la fois f7ij et f7il par exemple ?
                    P.max, maxi(f7id, f7ie, f7if, f7ig)
                )
                * taux_reduc_2012
    ...
    def formula_2015_01_01(foyer_fiscal, period, parameters):
        ...
        return (
            (
                ...
                * taux_reduc_2012
                + (min_(P.max, f7jt + f7ju) + min_(P.max, f7ou) + min_(P.max, f7ov))
                * taux_reduc_2013
            )
            / 9
            + report_invest_anterieur
    ...
    def formula_2017_01_01(foyer_fiscal, period, parameters):
        ...
        report_reduc_invest_2009_2010 = P.taux * max_(
            f7ik + f7ip, f7ir + f7iq
        )  # avant 2011, report de l'investissement et non de la réduction
        ...
        reduc_invest_acheves_2017 = P.taux18 * min_(
            P.max, invest_2011_acheves_2017
        ) + P.taux11 * min_(
            max0,
    ...
    def formula(foyer_fiscal, period, parameters):
        ...
        return (
            ...
            * (
                ...
                + max_(
                    0, (f7wn < f7wo) * (f7wo > P.seuil) * P.taux * P.seuil * f7wn / div
                )
            )
            + (f7wm != 0)
            * (
...
class reduction_impot_exceptionnelle(Variable):
    ...
    def formula_2013_01_01(foyer_fiscal, period, parameters):
        ...
        params = parameters(
            period
        ).impot_revenu.reductions_impots.reduction_impot_exceptionnelle
        plafond = (
            params.seuil * nb_adult + (nbptr - nb_adult) * 2 * params.majoration_seuil
        )
...
class rehab(Variable):
    ...
    entity = FoyerFiscal
    label = (
        u"Réduction d'impôt pour travaux de réhabilitation des résidences de tourisme"
    )
...
class resimm(Variable):
    ...
    def formula_2016_01_01(foyer_fiscal, period, parameters):
        ...
        return P.taux_rc * min_(f7sy + f7rf + f7nx, max1) + P.taux_re * min_(
            f7re + f7sx + f7ny, max2
        )
    ...
    def formula_2017_01_01(foyer_fiscal, period, parameters):
        ...
        return P.taux_30 * (
            min_(depenses_secteur_degrade, P.max) + min_(depenses_PSMV_2017, P.max2)
        ) + P.taux_22 * (
...
class rpinel(Variable):
    ...
    def formula_2014_01_01(foyer_fiscal, period, parameters):
        ...
        max1 = max_(
            0, P.seuil - invest_domtom_2014 - f7qd
        )  # 2014 : plafond commun 'duflot' et 'rpinel'
        max2 = max_(0, max1 - f7qc)
        max3 = max_(0, max2 - invest_metropole_2014 - f7qb)
...
class saldom(Variable):
    ...
    def formula_2009_01_01(foyer_fiscal, period, parameters):
        ...
        max_non_inv = min_(max_base + P.pac * nbpacmin, max_du_max_non_inv)
        max_non_inv2 = min_(
            max_base + P.pac * nb_pac_majoration_plafond, max_du_max_non_inv
        )
        max_effectif = max_non_inv * not_(invalide) + P.max3 * invalide
...
class scelli(Variable):
    ...
    entity = FoyerFiscal
    label = (
        u"Réduction d'impôt au titre des investissements locatifs - Dispositif Scellier"
    )
    ...
    def formula_2010_01_01(foyer_fiscal, period, parameters):
        ...
        return (
            ...
            / 9
            + max_(P.taux25 * min_(P.max, f7hl), P.taux40 * min_(P.max, f7hm)) / 9
        ...
        return (
            min_(
                ...
                maxi(
                    ...
                    P.taux40
                    * maxi(f7nl / 9, f7nm / 9, f7nn / 9, f7nq / 5, f7nr / 5, f7ns / 5),
                ),
            )
            + min_(
                P.max, maxi(P.taux25 * max_(f7hj, f7hn), P.taux40 * max_(f7hk, f7ho))
            )
            / 9
            ...
            + min_(
                P.max,
                maxi(
                    P.taux25 * f7hv, P.taux25 * f7hx, P.taux40 * f7hw, P.taux40 * f7hz
                ),
            )
        ...
        return (
            ...
            + min_(
                ...
                maxi(
                    ...
                    P.taux22 * maxi(f7jb, f7jd) / 9,
                    P.taux24
                    * maxi(f7jk / 9, f7jn / 9, f7jo / 5, f7jr / 5, f7fc / 9, f7fd / 5),
                    P.taux36 * maxi(f7jl / 9, f7jm / 9, f7jp / 5, f7jq / 5),
                ),
            )
            + f7la
    ...
    def formula_2014_01_01(foyer_fiscal, period, parameters):
        ...
        report_scelli_2009 = min_(
            P.max, max_(P.taux25 * f7hr, P.taux40 * f7hs)
        )  # to check si application plafond
        report_scelli_2010 = min_(
            P.max,
            maxi(P.taux25 * f7hv, P.taux25 * f7hx, P.taux40 * f7hw, P.taux40 * f7hz),
        ) + min_(
            P.max, max_(P.taux25 * f7ht, P.taux40 * f7hu)
        )  # to check si application plafond
        ...
        reduc_scelli_2014_invest_2010 = min_(
            P.max,
            maxi(P.taux25 * max_(f7hj, f7hn) / 9, P.taux40 * max_(f7hk, f7ho) / 9),
        )

        reduc_scelli_2014_invest_2011 = min_(
            P.max,
            maxi(
                P.taux13 * max_(f7nf, f7nj) / 9,
                P.taux15 * max_(f7ng, f7ni) / 9,
        ...
        reduc_scelli_2014_invest_2012_mars_2013 = min_(
            P.max,
            maxi(
                P.taux6 * maxi(f7jf, f7jj, f7fb) / 9,
                P.taux13 * maxi(f7ja, f7je, f7jg, f7jh, f7fa) / 9,
    ...
    def formula_2015_01_01(foyer_fiscal, period, parameters):
        ...
        reduc_scelli_2015_invest_2010 = min_(
            P.max,
            maxi(P.taux25 * max_(f7hj, f7hn) / 9, P.taux40 * max_(f7hk, f7ho) / 9),
        )

        reduc_scelli_2015_invest_2011 = min_(
            P.max,
            maxi(
                P.taux13 * max_(f7nf, f7nj) / 9,
                P.taux15 * max_(f7ng, f7ni) / 9,
        ...
        reduc_scelli_2015_invest_2012_mars_2013 = min_(
            P.max,
            maxi(
                P.taux6 * maxi(f7jf, f7jj, f7fb) / 9,
                P.taux13 * maxi(f7ja, f7je, f7jg, f7jh, f7fa) / 9,
    ...
    def formula_2016_01_01(foyer_fiscal, period, parameters):
        ...
        reduc_scelli_2016_invest_2010 = min_(
            P.max,
            maxi(P.taux25 * max_(f7hj, f7hn) / 9, P.taux40 * max_(f7hk, f7ho) / 9),
        )

        reduc_scelli_2016_invest_2011 = min_(
            P.max,
            maxi(
                P.taux13 * max_(f7nf, f7nj) / 9,
                P.taux15 * max_(f7ng, f7ni) / 9,
        ...
        reduc_scelli_2016_invest_2012_mars_2013 = min_(
            P.max,
            maxi(
                P.taux6 * maxi(f7jf, f7jj, f7fb) / 9,
                P.taux13 * maxi(f7ja, f7je, f7jg, f7jh, f7fa) / 9,
