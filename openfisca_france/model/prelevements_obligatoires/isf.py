...
class b1cb(Variable):
    ...
    entity = FoyerFiscal
    label = (
        u"Parts et actions de sociétés avec engagement de conservation de 6 ans minimum"
    )
    definition_period = YEAR
...
class isf_inv_pme(Variable):
    ...
    def formula_2008(foyer_fiscal, period, parameters):
        ...
        inv_dir_soc = (
            b2mt * P.taux_don_interet_general + b2ne * P.taux_invest_direct_soc_holding
        )
        holdings = (
            b2mv * P.taux_don_interet_general + b2nf * P.taux_invest_direct_soc_holding
        )
...
class isf_avant_plaf(Variable):
    ...
    def formula_2008(foyer_fiscal, period, parameters):
        ...
        return max_(
            0, isf_avant_reduction - (isf_inv_pme + isf_org_int_gen) - isf_reduc_pac
        )
    ...
    def formula_2009(foyer_fiscal, period, parameters):
        ...
        return max_(
            0,
            isf_avant_reduction
            - min_(isf_inv_pme + isf_org_int_gen, borne_max)
            - isf_reduc_pac,
        )
...
class revetproduits(Variable):
    ...
    def formula(foyer_fiscal, period, parameters):
        ...
        """
        salcho_imp_i = foyer_fiscal.members(
...
class isf_apres_plaf(Variable):
    ...
    def formula_2013_01_01(foyer_fiscal, period, parameters):
        ...
        plafond = max_(
            0, tot_impot - revetproduits
        )  # case PU sur la déclaration d'impôt
...
class isf_tot(Variable):
    ...
    def formula(foyer_fiscal, period, parameters):
        ...
        return min_(
            -(
                (isf_apres_plaf - b4rs) * ((-irpp) > 0)
                + (isf_avant_plaf - b4rs) * ((-irpp) <= 0)
            ),
            0,
        )
...
class bouclier_imp_gen(Variable):  # # ajouter CSG- CRDS
    ...
    def formula_2006(foyer_fiscal, period, parameters):
        ...
        csg_deductible_salaire_i = foyer_fiscal.members(
            "csg_deductible_salaire", period
        )
        ...
        csg_deductible_chomage_i = foyer_fiscal.members(
            "csg_deductible_chomage", period
        )
        csg_deductible_retraite_i = foyer_fiscal.members(
            "csg_deductible_retraite", period
        )
        csg_imposable_retraite_i = foyer_fiscal.members(
            "csg_imposable_retraite", period
        )
        imp_lib = foyer_fiscal("imp_lib", period)
        ...
        taxe_habitation = foyer_fiscal.sum(
            taxe_habitation, role=Menage.PERSONNE_DE_REFERENCE
        )
