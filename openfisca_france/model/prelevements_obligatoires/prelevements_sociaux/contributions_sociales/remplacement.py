...
class TypesTauxCSGRemplacement(Enum):
    __order__ = (
        "non_renseigne exonere taux_reduit taux_plein"
...
class csg_deductible_chomage(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        csg_deductible_chomage = max_(
            -montant_csg
            - max_(
                cho_seuil_exo - (chomage_brut + csg_imposable_chomage + montant_csg), 0
            ),
            0,
        )
...
class csg_imposable_chomage(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        csg_imposable_chomage = max_(
            -montant_csg - max_(cho_seuil_exo - (chomage_brut + montant_csg), 0), 0
        )
...
class crds_chomage(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        cho_seuil_exo = (
            law.prelevements_sociaux.contributions.csg.chomage.min_exo
            * heures_mensuelles
            * smic_h_b
        )
        eligible = (taux_csg_remplacement == TypesTauxCSGRemplacement.taux_reduit) + (
            taux_csg_remplacement == TypesTauxCSGRemplacement.taux_plein
        )
        montant_crds = (
            montant_csg_crds(
...
class chomage_imposable(Variable):
    value_type = float
    unit = "currency"
    cerfa_field = {0: u"1AP", 1: u"1BP", 2: u"1CP", 3: u"1DP", 4: u"1EP"}
    entity = Individu
    label = u"Allocations chômage imposables"
    set_input = set_input_divide_by_period
    reference = (
        u"http://www.insee.fr/fr/methodes/default.asp?page=definitions/chomage.htm"
    )
    definition_period = MONTH
...
class casa(Variable):
    ...
    def formula_2015_01_01(individu, period, parameters):
        ...
        casa = (
            ...
            * (
                rfr
                > contributions.csg.remplacement.pensions_de_retraite_et_d_invalidite.seuil_de_rfr_2
            )
...
class retraite_imposable(Variable):
    unit = "currency"
    value_type = float
    cerfa_field = {0: u"1AS", 1: u"1BS", 2: u"1CS", 3: u"1DS", 4: u"1ES"}
    entity = Individu
...
class crds_pfam(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        taux_crds = parameters(
            period.start
        ).prelevements_sociaux.contributions.crds.taux
