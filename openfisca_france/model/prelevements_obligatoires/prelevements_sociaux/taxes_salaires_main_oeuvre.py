...
class contribution_supplementaire_apprentissage(Variable):
    ...
    reference = (
        u"https://www.service-public.fr/professionnels-entreprises/vosdroits/F22574"
    )
    definition_period = MONTH
    ...
    def formula_2010_01_01(individu, period, parameters):
        ...
        if period.start.year > 2012:
            ...
            multiplier = (
                salarie_regime_alsace_moselle * csa_params.multiplicateur_alsace_moselle
            ...
            taux_conditionnel = (
                ...
                * (ratio_alternants < .01)
                * csa_params.moins_2000_moins_1pc_alternants
                + (effectif_entreprise >= 2000)
                * (ratio_alternants < .01)
                * csa_params.plus_2000_moins_1pc_alternants
                + (.01 <= ratio_alternants)
                * (ratio_alternants < .02)
                * csa_params.entre_1_2_pc_alternants
                + (.02 <= ratio_alternants)
                * (ratio_alternants < .03)
                * csa_params.entre_2_3_pc_alternants
                + (.03 <= ratio_alternants)
                * (ratio_alternants < .04)
                * csa_params.entre_3_4_pc_alternants
                + (.04 <= ratio_alternants)
                * (ratio_alternants < .05)
                * csa_params.entre_4_5_pc_alternants
            )
            taux_contribution = taxe_due * taux_conditionnel * multiplier
        else:
            taux_contribution = (
                effectif_entreprise >= 250
            ) * cotsoc_params.contribution_supplementaire_apprentissage.plus_de_250
...
class cotisations_employeur_main_d_oeuvre(Variable):
    ...
    def formula(individu, period, parameters):
        conge_individuel_formation_cdd = individu(
            "conge_individuel_formation_cdd", period
        )
        ...
        financement_organisations_syndicales = individu(
            "financement_organisations_syndicales", period
        )
        fnal = individu("fnal", period)
        formation_professionnelle = individu("formation_professionnelle", period)
        participation_effort_construction = individu(
            "participation_effort_construction", period, options=[ADD]
        )
        prevoyance_obligatoire_cadre = individu(
            "prevoyance_obligatoire_cadre", period, options=[ADD]
        )
        complementaire_sante_employeur = individu(
            "complementaire_sante_employeur", period, options=[ADD]
        )
...
class formation_professionnelle(Variable):
    ...
    reference = (
        u"https://www.service-public.fr/professionnels-entreprises/vosdroits/F22570"
    )
    definition_period = MONTH
...
class participation_effort_construction(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        cotisation = bareme * (effectif_entreprise >= 20) + individu.filled_array(0) * (
...
class taxe_salaires(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        complementaire_sante_employeur = individu(
            "complementaire_sante_employeur", period
        )
        ...
        entreprise_est_association_non_lucrative = individu(
            "entreprise_est_association_non_lucrative", period
        )
        effectif_entreprise = individu("effectif_entreprise", period)
        ...
        assujettissement = (
            assujettie_taxe_salaires + entreprise_est_association_non_lucrative
        )
        ...
        cotisation_individuelle = bareme.calc(
            base, factor=1 / 12, round_base_decimals=2
        ...
        conditions = [
            estimation < parametres.franchise,
            estimation <= parametres.decote_montant,
            estimation > parametres.decote_montant,
        ]
        results = [
            0,
            estimation
            - (parametres.decote_montant - estimation) * parametres.decote_taux,
            estimation,
        ]
