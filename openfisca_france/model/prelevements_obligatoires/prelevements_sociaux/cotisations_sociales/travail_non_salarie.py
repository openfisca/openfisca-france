...
class formation_artisan_commercant(Variable):
    ...
    def formula_2015(individu, period, parameters):
        ...
        return -bareme_artisant.calc(assiette * artisan) - bareme_commercant.calc(assiette * commercant)
...
class retraite_complementaire_artisan_commercant(Variable):
    ...
    def formula_2013(individu, period, parameters):
        plafond_securite_sociale_annuel = parameters(period).cotsoc.gen.plafond_securite_sociale * 12
        retraite_complementaire = parameters(period).ret_comp_ac.artisans_industriels_commercants.rci
        ...
        bareme.add_bracket(montant_du_plafond_rci, entre_1_plafond_rci_et_4_plafonds_pss)
...
class vieillesse_artisan_commercant(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        bareme.add_bracket(0, vieillesse_artisan_commercant.artisans)
        bareme.add_bracket(1, vieillesse_artisan_commercant.tous_independants)
        bareme.multiply_thresholds(plafond_securite_sociale_annuel)
        categorie_non_salarie = individu('categorie_non_salarie', period)
        assiette = (
            (categorie_non_salarie == TypesCategorieNonSalarie.artisan)
...
class famille_travailleur_independant(Variable):
    value_type = float
    entity = Individu
...
class formation_professionelle_profession_liberale(Variable):
    value_type = float
    entity = Individu
...
class vieillesse_profession_liberale(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        assurance_vieillesse = parameters.prelevements_sociaux.ret_pl.assurance_vieillesse
