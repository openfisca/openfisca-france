...
class pension_civile_salarie(Variable):
    ...
    def formula(individu, period, parameters):
        traitement_indiciaire_brut = individu(
        ...
        pension_civile_salarie = (
            categorie_salarie == TypesCategorieSalarie.public_titulaire_etat
        ) * sal["public_titulaire_etat"]["pension"].calc(
            traitement_indiciaire_brut
        ) + terr_or_hosp * sal[
...
class pension_civile_employeur(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        cot_pat_pension_civile = (
            categorie_salarie == TypesCategorieSalarie.public_titulaire_etat
        ) * pat["public_titulaire_etat"]["pension"].calc(
...
class rafp_salarie(Variable):
    ...
    def formula_2005_01_01(individu, period, parameters):
        ...
        assiette = min_(
            base_imposable, plaf_ass * traitement_indiciaire_brut * eligible
        )
        # Même régime pour les fonctions publiques d'Etat et des collectivité locales
        rafp_salarie = eligible * _P.cotsoc.cotisations_salarie.public_titulaire_etat[
            "rafp"
        ].calc(assiette)
        return -rafp_salarie
...
def seuil_fds(law):
    ...
    seuil_mensuel = math.floor(
        (pt_ind_mensuel * fds.indice_majore_de_reference)
    )  # TODO improve
