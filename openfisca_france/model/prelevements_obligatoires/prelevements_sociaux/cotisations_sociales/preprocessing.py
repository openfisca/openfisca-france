...
def build_pat(node_json):
    ...
    for bareme in [
        "apprentissage",
        "apprentissage_add",
        "apprentissage_alsace_moselle",
    ]:
        commun.children[bareme] = commun.children["apprentissage_node"].children[bareme]
    del commun.children["apprentissage_node"]

    commun.children["formprof_09"] = commun.children["formprof_node"].children[
        "formprof_09"
    ]
    commun.children["formprof_1019"] = commun.children["formprof_node"].children[
        "formprof_1019"
    ]
    commun.children["formprof_20"] = commun.children["formprof_node"].children[
        "formprof_20"
    ]
    del commun.children["formprof_node"]

    commun.children["construction"] = commun.children["construction_node"].children[
        "construction_20"
    ]
    ...
    pat.children["public_titulaire_hospitaliere"] = copy.deepcopy(
        pat.children["public_titulaire_territoriale"]
    )
    for category in ["territoriale", "hospitaliere"]:
        for name, bareme in (
            pat.children["public_titulaire_" + category]
            .children[category]
            .children.items()
        ):
...
def build_sal(node_json):
    ...
    sal.children["public_titulaire_territoriale"] = sal.children["fonc"].children[
        "colloc"
    ]
    sal.children["public_titulaire_hospitaliere"] = sal.children["fonc"].children[
        "colloc"
    ]
    ...
    for type_sal_category in [
        ...
        sal.children[type_sal_category].children["excep_solidarite"] = (
            sal.children["fonc"].children["commun"].children["solidarite"]
        )
    ...
    for type_sal_category in [
        ...
        sal.children[type_sal_category].children["rafp"] = (
            sal.children["fonc"].children["etat"].children["rafp"]
        )

    sal.children["public_non_titulaire"].children.update(
        sal.children["commun"].children
    )
...
def preprocess_parameters(parameters):
    ...
    cotsoc.children["cotisations_employeur"] = ParameterNode(
        "cotisations_employeur_after_preprocessing", data={}
    )
    cotsoc.children["cotisations_salarie"] = ParameterNode(
        "cotisations_salarie_after_preprocessing", data={}
    )
