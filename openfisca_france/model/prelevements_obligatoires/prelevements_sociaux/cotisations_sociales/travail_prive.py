...
class assiette_cotisations_sociales(Variable):
    ...
    def formula(individu, period, parameters):
        assiette_cotisations_sociales_prive = individu(
            "assiette_cotisations_sociales_prive", period
        )
        assiette_cotisations_sociales_public = individu(
            "assiette_cotisations_sociales_public", period
        )
        categorie_salarie = individu("categorie_salarie", period)
        stage_gratification_reintegration = individu(
...
class assiette_cotisations_sociales_prive(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        indemnites_compensatrices_conges_payes = individu(
            "indemnites_compensatrices_conges_payes", period
        )
...
class reintegration_titre_restaurant_employeur(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        cantines_titres_restaurants = parameters(
            period
        ).cotsoc.assiette.cantines_titres_restaurants
        ...
        condition_exoneration_taux = (taux_minimum_exoneration <= taux_employeur) * (
...
class penibilite(Variable):
    ...
    def formula_2015_01_01(individu, period, parameters):
        ...
        multiplicateur = parameters(
            period
        ).cotsoc.cotisations_employeur.prive_cadre.penibilite_multiplicateur_exposition_multiple
        ...
        cotisation = switch(
            exposition_penibilite,
            {
                ...
                TypesExpositionPenibilite.simple: cotisation_base
                + cotisation_additionnelle,
                TypesExpositionPenibilite.multiple: cotisation_base
                + cotisation_additionnelle * multiplicateur,
            },
        )
...
class accident_du_travail(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        categorie_salarie = individu("categorie_salarie", period)
        assujetti = (categorie_salarie == TypesCategorieSalarie.prive_non_cadre) + (
            categorie_salarie == TypesCategorieSalarie.prive_cadre
        )
...
class agirc_gmp_assiette(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        assiette = max_(
            (salaire_charniere - assiette_cotisations_sociales)
...
class agirc_gmp_salarie(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        cotisation_forfaitaire = (
            gmp.cotisation_forfaitaire_mensuelle_en_euros.part_salariale
        )

        taux = parameters(period).cotsoc.cotisations_salarie.prive_cadre.agirc.rates[1]
        sous_plafond_securite_sociale = (
            assiette_cotisations_sociales <= plafond_securite_sociale
        ) & (assiette_cotisations_sociales > 0)
        cotisation = -(
        ...
        return min_(
            (cotisation - agirc_salarie)
            * (categorie_salarie == TypesCategorieSalarie.prive_cadre),
            0,
        )  # cotisation are negative
...
class agirc_gmp_employeur(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        gmp = parameters(period).prelevements_sociaux.gmp
        cotisation_forfaitaire = (
            gmp.cotisation_forfaitaire_mensuelle_en_euros.part_patronale
        )
        taux = (
            parameters(period)
            .cotsoc.cotisations_employeur["prive_cadre"]["agirc"]
            .rates[1]
        )
...
class apec_salarie(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        return cotisation * (
            categorie_salarie == TypesCategorieSalarie.prive_cadre
        )  # TODO: check public notamment contractuel
...
class arrco_salarie(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        public = (categorie_salarie == TypesCategorieSalarie.prive_non_cadre) + (
            categorie_salarie == TypesCategorieSalarie.prive_cadre
        )
...
class mmid_salarie(Variable):
    ...
    def formula(individu, period, parameters):
        salarie_regime_alsace_moselle = individu(
        ...
        cotisation = (
            ...
            + salarie_regime_alsace_moselle * cotisation_regime_alsace_moselle
        )
...
class mmida_employeur(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        contribution_solidarite_autonomie = individu(
            "contribution_solidarite_autonomie", period
        )
    ...
    def formula(individu, period, parameters):
        ...
        plafond = switch(
            contrat_de_travail,
            {
                ...
                TypesContratDeTravail.temps_partiel: plafond_temps_plein
                * (heures_remunerees_volume / heures_temps_plein),
                TypesContratDeTravail.forfait_jours_annee: plafond_temps_plein
                * (forfait_jours_remuneres_volume / 218),
...
class complementaire_sante_salarie(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        cotisation = (
            -(1 - complementaire_sante_taux_employeur) * complementaire_sante_montant
        )
        return cotisation
...
class TypesTailleEntreprise(Enum):
    __order__ = (
        "non_pertinent moins_de_10 de_10_a_19 de_20_a_249 plus_de_250"
...
class taux_accident_travail(Variable):
    ...
    def formula_2012_01_01(individu, period, parameters):
        ...
        return (
            (exposition_accident == TypesExpositionAccident.faible) * accident.faible
            + (exposition_accident == TypesExpositionAccident.moyen) * accident.moyen
            + (exposition_accident == TypesExpositionAccident.eleve) * accident.eleve
