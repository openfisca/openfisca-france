...
from numpy import (
    busday_count as original_busday_count,
    datetime64,
    logical_or as or_,
    logical_and as and_,
    timedelta64,
...
class coefficient_proratisation(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        forfait_heures_remunerees_volume = individu(
            "forfait_heures_remunerees_volume", period
        )
        ...
        busday_count = partial(
            original_busday_count, holidays=holidays
        )  # @holidays : jours feriés français

        debut_mois = datetime64(period.start.offset("first-of", "month"))
        fin_mois = datetime64(period.start.offset("last-of", "month")) + timedelta64(
            1, "D"
        )  # busday ignores the last day

        jours_ouvres_ce_mois = busday_count(debut_mois, fin_mois, weekmask="1111100")

        mois_incomplet = or_(
            contrat_de_travail_debut > debut_mois, contrat_de_travail_fin < fin_mois
        )
        ...
        coefficient = switch(
            ...
            {  # temps plein
                ...
                * (
                    ...
                    / (
                        jours_ouvres_ce_mois * coefficient_proratisation_temps_partiel
                        + 1e-16
                    )
                ),
                ...
                * (
                    ...
                    / (
                        jours_ouvres_ce_mois * coefficient_proratisation_forfait_jours
                        + 1e-16
                    )
                ),
            },
    ...
    def formula_2015_06_09(individu, period, parameters):
        ...
        eligible_duree = or_(
            ...
            and_(
                ...
                # > 6 mois
                (contrat_de_travail_fin - contrat_de_travail_debut).astype(
                    "timedelta64[M]"
                )
                >= timedelta64(6, "M")
        ...
        eligible_date = (
            datetime64(period.offset(-24, "month").start) < contrat_de_travail_debut
        )
        eligible = (
...
class allegement_fillon(Variable):
    ...
    reference = (
        u"https://www.service-public.fr/professionnels-entreprises/vosdroits/F24542"
    )
    definition_period = MONTH
    calculate_output = calculate_output_add
    ...
    def formula_2005_07_01(individu, period, parameters):
        ...
        allegement_mode_recouvrement = individu(
            "allegement_fillon_mode_recouvrement", period
        )
        exoneration_cotisations_employeur_jei = individu(
            "exoneration_cotisations_employeur_jei", period
        )
...
def compute_allegement_fillon(individu, period, parameters):
    ...
    if date(2003, 7, 1) <= period.start.date <= date(2005, 6, 30):
        seuil = (
            fillon.entreprises_ayant_signe_un_accord_de_rtt_avant_le_30_06_2003.plafond
        )
        tx_max = (
            fillon.entreprises_ayant_signe_un_accord_de_rtt_avant_le_30_06_2003.reduction_maximale
        )
    # Après le 2005-07-01
    else:
...
def compute_allegement_cotisation_allocations_familiales(individu, period, parameters):
    ...
    law = parameters(
        period
    ).prelevements_sociaux.allegement_cotisation_allocations_familiales

    # Montant de l'allegment
    return (
        (assiette < law.plafond_en_nombre_de_smic * smic_proratise)
        * law.reduction
        * assiette
    )
...
def compute_allegement_anticipe(
    individu, period, parameters, variable_name, compute_function
):
    if period.start.month < 12:
    ...
    if period.start.month == 12:
        ...
        return compute_function(individu, period.this_year, parameters) - cumul


def compute_allegement_progressif(
    individu, period, parameters, variable_name, compute_function
):
    ...
    if period.start.month > 1:
        up_to_this_month = period.start.offset("first-of", "year").period(
            "month", period.start.month
        )
        up_to_previous_month = period.start.offset("first-of", "year").period(
            "month", period.start.month - 1
        )
        cumul = individu(
            variable_name, up_to_previous_month, max_nb_cycles=1, options=[ADD]
        )
...
def taux_exo_cice(assiette_allegement, smic_proratise, parameters):
    ...
    taux_cice = (
        (assiette_allegement / (smic_proratise + 1e-16)) <= cice.plafond_smic
    ) * cice.taux
    return taux_cice
