...
def compute_allegement_cotisation_allocations_familiales_base(individu, period, parameters):
    ...
    assiette = individu('assiette_allegement', period)
    smic_proratise = individu('smic_proratise', period)
    P = parameters(period).prelevements_sociaux.reductions_cotisations_sociales.allegement_cotisation_allocations_familiales
    taux_reduction = P.reduction
    if period.start.year < 2024:
        plafond_reduction = P.plafond_smic * smic_proratise
    else:
        coefficient_proratisation = individu('coefficient_proratisation', period)
        ...
        plafond_reduction = max_(P.plafond_smic_courant * smic_proratise, P.plafond_smic_2023_12_31 * smic_proratise_2O23_12)
