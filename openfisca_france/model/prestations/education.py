...
class bourse_college_echelon(Variable):
    ...
    def formula_2016_07_01(famille, period, parameters):
        ...
        P = parameters(period).prestations.aides_jeunes.bourses.bourses_education.bourse_college.apres_2016
    ...
    def formula(famille, period, parameters):
        ...
        P = parameters(period).prestations.aides_jeunes.bourses.bourses_education.bourse_college.avant_2016
...
class bourse_college(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        """
        P = parameters(period).prestations.aides_jeunes.bourses.bourses_education.bourse_college

        # On prends en compte la BMAF du premier janvier de l'année de la rentrée scolaire
        bmaf_1er_janvier = parameters(period.this_year.first_month).prestations.prestations_familiales.prestations_generales.af.bmaf
...
class bourse_lycee_nombre_parts(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        rfr = famille.demandeur.foyer_fiscal('rfr', period.n_2)
        plafonds_reference = parameters(period).prestations.aides_jeunes.bourses.bourses_education.bourse_lycee.avant_2016.plafonds_reference
        increments_par_point_de_charge = parameters(period).prestations.aides_jeunes.bourses.bourses_education.bourse_lycee.avant_2016.increments_par_point_de_charge
...
class bourse_lycee_echelon(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        age_i = famille.members('age', period)
        nb_enfants = famille.sum(age_i >= 0, role = Famille.ENFANT)
        P = parameters(period).prestations.aides_jeunes.bourses.bourses_education.bourse_lycee.apres_2016
...
class bourse_lycee(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        valeur_part = parameters(period).prestations.aides_jeunes.bourses.bourses_education.bourse_lycee.avant_2016.valeur_part
...
class detention_carte_des_metiers(Variable):
    ...
    def formula(individu, period, parameters):
        return individu('alternant', period) * (individu('age', period) < parameters(period).prestations.aides_jeunes.carte_des_metiers.age_maximal)
