...
from openfisca_france.model.prestations.prestations_familiales.base_ressource import (
    nb_enf
)
...
class al_nb_personnes_a_charge(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        * vos proches parents infirmes âgés de 22 ans ou plus (parents,
        ...
        age_max_enfant = parameters(
            period
        ).prestations.prestations_familiales.cf.age_max
        residence_dom = famille.demandeur.menage("residence_dom", period)

        def al_nb_enfants():
            age_min_enfant = parameters(
                period
            ).prestations.prestations_familiales.af.age1
            return nb_enf(
                famille, period, age_min_enfant, age_max_enfant - 1
            )  # La limite sur l'age max est stricte.

        def al_nb_adultes_handicapes():

            # Variables à valeur pour un individu
            base_ressources_i = famille.members(
            ...
            plafond_ressource = (
                parameters(
                    period.n_2.stop
                ).prestations.minima_sociaux.aspa.plafond_ressources_seul
                * 1.25
            )
...
class aide_logement_base_ressources_patrimoine(Variable):
    ...
    def formula_2016_10_01(famille, period, parameters):
        ...
        epargne_revenus_non_imposables_i = famille.members(
            "epargne_revenus_non_imposables", period
        )
        epargne_revenus_non_imposables = famille.sum(epargne_revenus_non_imposables_i)
        epargne_revenus_imposables_i = famille.members(
            "epargne_revenus_imposables", period
        )
        ...
        valeur_terrains_non_loues_i = famille.members(
            "valeur_terrains_non_loues", period
        )
        valeur_terrains_non_loues = famille.sum(valeur_terrains_non_loues_i)

        valeur_locative_immo_non_loue_i = famille.members(
            "valeur_locative_immo_non_loue", period
        )
        valeur_locative_immo_non_loue = famille.sum(valeur_locative_immo_non_loue_i)
        valeur_locative_terrains_non_loues_i = famille.members(
            "valeur_locative_terrains_non_loues", period
        )
        valeur_locative_terrains_non_loues = famille.sum(
            valeur_locative_terrains_non_loues_i
        )
        ...
        foncier = (
            valeur_patrimoine_loue + valeur_immo_non_loue + valeur_terrains_non_loues
        )
        ...
        est_locataire_foyer = (
            statut_logement == TypesStatutOccupationLogement.locataire_foyer
        )
        ...
        seuil_valorisation = parameters(
            period
        ).prestations.aides_logement.patrimoine.seuil_valorisation

        return (
            not_(est_locataire_foyer)
            * not_(famille_percoit_aah)
            * not_(famille_percoit_aeeh)
            * (
                (patrimoine > seuil_valorisation)
                * (
                    +capitaux_non_productifs
                    * abattements.taux_interet_forfaitaire_epargne_non_imposable
                    + valeur_locative_immo_non_loue
                    * abattements.abattement_valeur_locative_immo_non_loue
                    + valeur_locative_terrains_non_loues
                    * abattements.abattement_valeur_locative_terrains_non_loues
                )
            )
        )
...
class aide_logement_base_ressources_eval_forfaitaire(Variable):
    ...
    def formula(famille, period, parameters):
        def eval_forfaitaire_salaries():
            salaire_imposable_i = famille.members(
                "salaire_imposable", period.offset(-1)
            )
            ...
            montant_abattement = round_(
                min_(
                    max_(
                        params_abattement.taux * somme_salaires_mois_precedent,
                        params_abattement.min,
                    ),
        ...
        def eval_forfaitaire_tns():
            ...
            travailleur_non_salarie_i = famille.members(
                "travailleur_non_salarie", period
            )
...
class aide_logement_assiette_abattement_chomage(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        abattement_forfaitaire = round_(
            min_(max_(abatpro.taux * revenu_salarie, abattement_minimum), abatpro.max)
        )
...
class aide_logement_abattement_chomage_indemnise(Variable):
    ...
    entity = Individu
    label = (
        u"Montant de l'abattement pour personnes au chômage indemnisé (R351-13 du CCH)"
    )
    definition_period = MONTH
    ...
    def formula(individu, period, parameters):
        ...
        revenus_activite_pro = individu(
            "aide_logement_assiette_abattement_chomage", period.n_2
        )
        taux_abattement = parameters(
            period
        ).prestations.aides_logement.ressources.abattement_chomage_indemnise
...
class aide_logement_abattement_depart_retraite(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        revenus_activite_pro = individu(
            "revenu_assimile_salaire_apres_abattements", period.n_2
        )
...
class aide_logement_neutralisation_rsa(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        revenus_a_neutraliser_i = famille.members(
            "revenu_assimile_salaire_apres_abattements", period.n_2
        )
...
class aide_logement_base_ressources_defaut(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        ressources_patrimoine = famille(
            "aide_logement_base_ressources_patrimoine", period
        )
        abattement_chomage_indemnise_i = famille.members(
            "aide_logement_abattement_chomage_indemnise", period
        )
        abattement_chomage_indemnise = famille.sum(
            abattement_chomage_indemnise_i, role=Famille.PARENT
        )
        abattement_depart_retraite_i = famille.members(
            "aide_logement_abattement_depart_retraite", period
        )
        abattement_depart_retraite = famille.sum(
            abattement_depart_retraite_i, role=Famille.PARENT
        )
        neutralisation_rsa = famille("aide_logement_neutralisation_rsa", period)
        abattement_ressources_enfant = (
            parameters(
                period.n_2.stop
            ).prestations.minima_sociaux.aspa.plafond_ressources_seul
            * 1.25
        ...
        base_ressources_enfants = famille.sum(
            max_(0, base_ressources_i - abattement_ressources_enfant),
            role=Famille.ENFANT,
        )

        demandeur_declarant_principal = famille.demandeur.has_role(
            FoyerFiscal.DECLARANT_PRINCIPAL
        )
        conjoint_declarant_principal = famille.conjoint.has_role(
            FoyerFiscal.DECLARANT_PRINCIPAL
        )

        # Revenus du foyer fiscal
        aide_logement_base_revenus_fiscaux = (
            famille.demandeur.foyer_fiscal(
                "aide_logement_base_revenus_fiscaux", period.n_2
            )
            * demandeur_declarant_principal
            + famille.conjoint.foyer_fiscal(
                "aide_logement_base_revenus_fiscaux", period.n_2
            )
        ...
        ressources = (
            ...
            + aide_logement_base_revenus_fiscaux
            - (
                abattement_chomage_indemnise
                + abattement_depart_retraite
                + neutralisation_rsa
            )
        )
...
class aide_logement_base_revenus_fiscaux(Variable):
    ...
    def formula(foyer_fiscal, period):
        rente_viagere_titre_onereux_net = foyer_fiscal(
            "rente_viagere_titre_onereux_net", period
        )
        pensions_alimentaires_versees = foyer_fiscal(
        ...
        apply_abat_spe = (abat_spe > 0) * (
            invV + invC + (naissanceP < dateLimite) + (naissanceC < dateLimite)
        )
...
class aide_logement_base_ressources(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        plafond_salaire_jeune_isole = parameters(
            period
        ).prestations.aides_logement.ressources.dar_8
        plafond_salaire_jeune_couple = parameters(
            period
        ).prestations.aides_logement.ressources.dar_9
        plafond_salaire_jeune = where(
            en_couple, plafond_salaire_jeune_couple, plafond_salaire_jeune_isole
        )
        ...
        ressources = where(
            eval_forfaitaire, base_ressources_eval_forfaitaire, base_ressources_defaut
        )
...
class aide_logement_loyer_plafond(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        plafond_famille = (
            loyers_plafond.un_enfant
            + (al_nb_pac > 1) * (al_nb_pac - 1) * loyers_plafond.majoration_par_enf_supp
        )
...
class aide_logement_charges(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        coloc = famille.demandeur.menage("coloc", period)
        montant_coloc = (
            where(couple, 1, 0.5) * P.cas_general + al_nb_pac * P.majoration_par_enfant
        )
...
class aide_logement_R0(Variable):
    ...
    def formula(famille, period, parameters):
        al = parameters(period).prestations.aides_logement
        pfam_n_2 = parameters(
            period.start.offset(-2, "year")
        ).prestations.prestations_familiales
        minim_n_2 = parameters(
            period.start.offset(-2, "year")
...
class aide_logement_taux_famille(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        TF_metropole = (
            ...
            + al.taux_participation_fam.taux_enf_supp
            * (al_nb_pac > 4)
            * (al_nb_pac - 4)
        )
...
class aide_logement_taux_loyer(Variable):
    ...
    entity = Famille
    label = (
        u"Taux obscur basé sur une comparaison du loyer retenu à un loyer de référence."
    )
    definition_period = MONTH
    ...
    def formula(famille, period, parameters):
        ...
        TL = where(
            RL >= 0.75,
            al.taux_participation_loyer.taux_tranche_3 * (RL - 0.75)
            + al.taux_participation_loyer.taux_tranche_2 * (0.75 - 0.45),
...
class aide_logement_participation_personnelle(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        P0 = max_(
            al.participation_min.taux * E, al.participation_min.montant_forfaitaire
...
class aide_logement_montant_brut_avant_degressivite(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        locataire = (
            ...
            + (
                statut_occupation_logement
                == TypesStatutOccupationLogement.locataire_foyer
            )
        )
        accedant = (
        ...
        loyer_retenu = famille("aide_logement_loyer_retenu", period)
        charges_retenues = famille("aide_logement_charges", period)
        participation_personnelle = famille(
            "aide_logement_participation_personnelle", period
        )

        montant_locataire = max_(
            0, loyer_retenu + charges_retenues - participation_personnelle
        )
        ...
        montant = montant * (
            montant >= al.al_min.montant_min_mensuel.montant_min_apl_al
...
class aide_logement_montant_brut(Variable):
    ...
    reference = (
        u"https://www.legifrance.gouv.fr/eli/decret/2017/9/28/TERL1721632D/jo/texte"
    )
    definition_period = MONTH

    def formula(famille, period):
        montant_avant_degressivite = famille(
            "aide_logement_montant_brut_avant_degressivite", period
        )
    ...
    def formula_2016_07_01(famille, period):
        ...
        coeff = select(
            [
                loyer_reel <= loyer_degressivite,
                loyer_reel <= loyer_suppression,
                loyer_reel > loyer_suppression,
            ],
            [
                1,
                ...
                - (
                    (loyer_reel - loyer_degressivite)
                    / (loyer_suppression - loyer_degressivite)
                ),
                0,
            ],
        )
    ...
    def formula_2017_10_01(famille, period, parameters):
        ...
        montant_avant_degressivite_et_coeff = round_(
            montant_avant_degressivite * coeff, 2
        )

        abattement_forfaitaire = parameters(
            period
        ).prestations.aides_logement.autres.abattement_forfaitaire
        aide_logement_apres_abattement_forfaitaire = (
            montant_avant_degressivite_et_coeff > 0
        ) * (montant_avant_degressivite_et_coeff - abattement_forfaitaire)
...
class TypesZoneApl(Enum):
    __order__ = (
        "non_renseigne zone_1 zone_2 zone_3"
    )  # Needed to preserve the enum order in Python 2
...
class zone_apl(Variable):
    ...
    def formula(menage, period):
        ...
        zone = fromiter(
            (
                zone_apl_by_depcom.get(
                    depcom_cell
                    if isinstance(depcom_cell, str)
                    else depcom_cell.decode("utf-8"),
                    default_value,
                )
        ...
        return select(
            ...
            (
                TypesZoneApl.zone_1.index,
                TypesZoneApl.zone_2.index,
                TypesZoneApl.zone_3.index,
            ),
        )
...
def preload_zone_apl():
    ...
    if zone_apl_by_depcom is None:
        with pkg_resources.resource_stream(
            openfisca_france.__name__, "assets/apl/20110914_zonage.csv"
        ) as csv_file:
        ...
        with pkg_resources.resource_stream(
            ...
            for (
                subcommune_depcom,
                commune_depcom,
            ) in commune_depcom_by_subcommune_depcom.items():
                zone_apl_by_depcom[subcommune_depcom] = zone_apl_by_depcom[
                    commune_depcom
                ]
...
class aides_logement_primo_accedant(Variable):
    ...
    def formula_2007_07(famille, period, parameters):
        loyer = famille.demandeur.menage("loyer", period)
        plafond_mensualite = famille(
            "aides_logement_primo_accedant_plafond_mensualite", period
        )
...
class aides_logement_primo_accedant_k(Variable):
    ...
    def formula(famille, period, parameters):
        coef_k = parameters(
            period
        ).prestations.al_param_accal.constante_du_coefficient_k
...
class aides_logement_primo_accedant_nb_part(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        return (
            prestations.al_param_accal.n_0_personnes_a_charge.isole
            * not_(couple)
            * (al_nb_pac == 0)
            + prestations.al_param_accal.n_0_personnes_a_charge.menage
            * couple
            * (al_nb_pac == 0)
            + prestations.al_param.parametre_n["1_personne_a_charge"] * (al_nb_pac == 1)
            + prestations.al_param.parametre_n["2_personnes_a_charge"]
            * (al_nb_pac == 2)
            + prestations.al_param.parametre_n["3_personnes_a_charge"]
            * (al_nb_pac == 3)
            + prestations.al_param.parametre_n["4_personnes_a_charge"]
            * (al_nb_pac >= 4)
            + prestations.al_param.majoration_n_par_personne_a_charge_supplementaire
            * (al_nb_pac > 4)
            * (al_nb_pac - 4)
        )
...
class aides_logement_primo_accedant_ressources(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        loyer = famille.demandeur.menage("loyer", period)
        coef_plancher_ressources = parameters(
            period
        ).prestations.aides_logement.ressources.dar_3
