...
class aide_logement_montant_brut_crds(Variable):
    ...
    def formula_2018(famille, period, parameters):
        rls = parameters(period).prestations.aides_logement.reduction_loyer_solidarite
...
class al_nb_personnes_a_charge(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        * vos proches parents infirmes âgés de 22 ans ou plus (parents,
        ...
        age_max_enfant = parameters(period).prestations.prestations_familiales.prestations_generales.cf.age_max
        residence_dom = famille.demandeur.menage('residence_dom', period)

        def al_nb_enfants():
            age_min_enfant = parameters(period).prestations.prestations_familiales.prestations_generales.af.age1
        ...
        def al_nb_adultes_handicapes():
            ...
            # Parametres
            plafond_ressource = parameters(period.n_2.stop).prestations.solidarite_insertion.minimum_vieillesse.aspa.plafond_ressources.personnes_seules * 1.25
...
class aide_logement_base_ressources_patrimoine(Variable):
    ...
    def formula_2016_10_01(famille, period, parameters):
        ...
        abattements = parameters(period).prestations.solidarite_insertion.minima_sociaux.rsa.patrimoine
...
class aide_logement_base_ressources(Variable):
    ...
    def formula_2021_01_01(famille, period, parameters):
        ...
        abattement_ressources_enfant = parameters(period.n_2.stop).prestations.solidarite_insertion.minimum_vieillesse.aspa.plafond_ressources.personnes_seules * 1.25
...
class aide_logement_charges(Variable):
    ...
    def formula(famille, period, parameters):
        forfait_charges = parameters(period).prestations.aides_logement.forfait_charges
        al_nb_pac = famille('al_nb_personnes_a_charge', period)
        couple = famille('al_couple', period)
        coloc = famille.demandeur.menage('coloc', period)
...
class aide_logement_R0(Variable):
    ...
    def formula(famille, period, parameters):
        al = parameters(period).prestations.aides_logement
        pfam_n_2 = parameters(period.start.offset(-2, 'year')).prestations.prestations_familiales.prestations_generales
        minim_n_2 = parameters(period.start.offset(-2, 'year')).prestations.solidarite_insertion.minima_sociaux
        couple = famille('al_couple', period)
        al_nb_pac = famille('al_nb_personnes_a_charge', period)
        residence_dom = famille.demandeur.menage('residence_dom', period)
...
class crds_logement(Variable):
    ...
    def formula(famille, period, parameters):
        aide_logement_montant_brut = famille('aide_logement_montant_brut_crds', period)
        crds = parameters(period).prestations.prestations_familiales.prestations_generales.af.crds
