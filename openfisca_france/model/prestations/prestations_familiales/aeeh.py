...
class aeeh(DatedVariable):
    ...
    def function_20020101_20021231(self, simulation, period):
        ...
        Une majoration est versée au parent isolé bénéficiaire d'un complément d'Aeeh lorsqu'il cesse ou réduit son
        activité professionnelle ou lorsqu'il embauche une tierce personne rémunérée.
        '''
        period = period.start.offset('first-of', 'month').period('year')
        age_holder = simulation.compute('age', period)
        handicap_holder = simulation.compute('handicap', period)
