...
class asf_elig_enfant(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        eligibilite = (
            (age >= pfam.af.age1)
            * (age < pfam.af.age3)
            * not_(  # Âge compatible avec les prestations familiales
                autonomie_financiere
            )
        )  # Ne perçoit pas plus de ressources que "55% du SMIC" au sens CAF
...
class asf_elig(Variable):
    ...
    def formula(famille, period):
        pensions_alimentaires_percues = famille.members(
        ...
        return (
            not_(residence_mayotte) * isole * pas_de_pensions
