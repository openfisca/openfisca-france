...
class rsa_enfant_a_charge(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        if period.start.date >= date(2009, 6, 1):
            ...
            majo_rsa = P_rmi.rmi_maj.majoration_montant_max_rsa
...
class rsa_socle(Variable):
    ...
    def formula_2009_06_01(famille, period, parameters):
        ...
        taux = (
            1
            + (nb_personnes >= 2) * rsa.rsa_maj.majoration_rsa.taux_deuxieme_personne
            + (nb_personnes >= 3) * rsa.rsa_maj.majoration_rsa.taux_troisieme_personne
            + (nb_personnes >= 4) * where(nb_parents == 1, rsa.rsa_maj.majoration_rsa.taux_personne_supp, rsa.rsa_maj.majoration_rsa.taux_troisieme_personne)
...
class rsa_socle_majore(Variable):
    ...
    def formula_2009_06_01(famille, period, parameters):
        ...
        rsa = parameters(period).prestations_sociales.solidarite_insertion.minima_sociaux.rsa
        taux = rsa.rsa_maj.majoration_montant_max_rsa.pac0 + rsa.rsa_maj.majoration_montant_max_rsa.pac_enf_sup * nbenf
        socle = rsa.rsa_m.montant_de_base_du_rsa
