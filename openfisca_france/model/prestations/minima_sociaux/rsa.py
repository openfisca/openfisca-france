...
class rsa_base_ressources(Variable):
    ...
    def formula_2017_01_01(famille, mois_demande, parameters, mois_courant):
        rsa_base_ressources_prestations_familiales = famille(
            "rsa_base_ressources_prestations_familiales",
            mois_demande,
            extra_params=[mois_courant],
        )
        rsa_base_ressources_minima_sociaux = famille(
        ...
        ressources_individuelles_i = famille.members(
            "rsa_base_ressources_individu", mois_demande
        ) + famille.members(
            "rsa_revenu_activite_individu", mois_demande, extra_params=[mois_courant]
        )
...
class rsa_base_ressources_individu(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        types_revenus_pros = ["chomage_net", "retraite_nette"]
        ...
        neutral_max_forfaitaire = (
            3 * parameters(period).prestations.minima_sociaux.rmi.rmi
        )
        ...
        revenus_capitaux_prelevement_bareme = max_(
            0,
        ...
        revenus_capitaux_prelevement_liberatoire = max_(
            0,
            individu.foyer_fiscal(
        ...
        rente_viagere_titre_onereux = individu.foyer_fiscal(
            "rente_viagere_titre_onereux", period.last_3_months, options=[ADD]
        )
        ...
        revenus_foyer_fiscal_projetes = revenus_foyer_fiscal * individu.has_role(
...
class rsa_base_ressources_prestations_familiales(Variable):
    ...
    def formula_2002_01_01(famille, period):
        prestations = ["af_base", "cf", "asf", "apje", "ape"]
    ...
    def formula_2014_04_01(famille, period):
        ...
        prestations_calculees = ["rsa_forfait_asf", "paje_base"]
        prestations_autres = ["paje_clca", "paje_prepare", "paje_colca"]

        # On réinjecte le montant des prestations calculées
        result = sum(
            famille(prestation, period) for prestation in prestations_calculees
        )

        result += sum(
            famille(prestation, period.last_3_months, options=[ADD]) / 3
            for prestation in prestations_autres
        )
        ...
        result = (
            result + cf_non_majore + min_(af_base, af)
    ...
    def formula_2017_01_01(famille, mois_demande, parameters, mois_courant):
        ...
        # On réinjecte le montant des prestations calculées
        result = sum(
            famille(prestation, mois_demande) for prestation in prestations_calculees
        )

        result += sum(
            famille(prestation, mois_courant) for prestation in prestations_autres
        )
    ...
    def formula(individu, period, parameters):
        ...
        if period.start.date >= date(2017, 1, 1):
            ...
            ressources = (
                ...
                + individu("rsa_revenu_activite_individu", period, extra_params=[m_1])
                / 3
                + individu("rsa_revenu_activite_individu", period, extra_params=[m_2])
                / 3
                + individu("rsa_revenu_activite_individu", period, extra_params=[m_3])
                / 3
            )
        else:
            ressources = individu("rsa_base_ressources_individu", period) + (
                1 - P_rsa.pente
        ...
        def ouvre_droit_majoration():
            ...
            presence_autres_enfants = (
                famille.sum(enfant * not_(autonomie_financiere) * (age <= age_pac)) > 1
            )
        ...
        rsa_enf_charge = (
            ...
            * where(
                ...
                ressources
                < (majo_rsa.pac0 - 1 + majo_rsa.pac_enf_sup) * montant_base_rsa,
...
class rsa_indemnites_journalieres_activite(Variable):
    ...
    def formula(individu, period):
        ...
        condition_arret_recent = is_date_arret_de_travail_undefined * (
            ijss_activite_sous_condition(m_3) == 0
        )
...
class rsa_indemnites_journalieres_hors_activite(Variable):
    ...
    def formula(individu, period):
        return +individu("indemnites_journalieres", period) - individu(
            "rsa_indemnites_journalieres_activite", period
        )
...
class rsa_revenu_activite_individu(Variable):
    ...
    def formula_2017_01_01(individu, mois_demande, parameters, mois_courant):
        ...
        has_ressources_substitution = individu(
            "rsa_has_ressources_substitution", mois_demande
        )
        ...
        revenus_tns_annualises = individu(
            "ppa_rsa_derniers_revenus_tns_annuels_connus", mois_demande.this_year
        )

        revenus_non_moyennes = individu("primes_salaires_net", mois_courant) + individu(
            "indemnite_fin_contrat_net", mois_courant
        )
...
class rsa_fictif(Variable):
    ...
    def formula_2016_10(famille, mois_courant, parameters, mois_demande):
        ...
        rsa_base_ressources = famille(
            "rsa_base_ressources", mois_demande, extra_params=[mois_courant]
        )
...
class rsa_montant(Variable):
    value_type = float
    label = (
        u"Revenu de solidarité active, avant prise en compte de la non-calculabilité."
    )
    entity = Famille
    definition_period = MONTH
    ...
    def formula_2017_01_01(famille, period, parameters):
        ...
        rsa = (
            famille(
                "rsa_fictif", period.last_3_months, extra_params=[period], options=[ADD]
            )
            / 3
        )
...
class rsa_base_ressources_patrimoine_individu(Variable):
    ...
    def formula_2009_06_01(individu, period, parameters):
        ...
        epargne_revenus_non_imposables = individu(
            "epargne_revenus_non_imposables", period
        )
        revenus_capital = individu("revenus_capital", period)
        valeur_locative_immo_non_loue = individu(
            "valeur_locative_immo_non_loue", period
        )
        valeur_locative_terrains_non_loues = individu(
        ...
        return (
            ...
            * rsa.patrimoine.taux_interet_forfaitaire_epargne_non_imposable
            / 12
            + revenus_capital
            + revenus_locatifs
...
class rsa_condition_nationalite(Variable):
    ...
    def formula_2009_06_01(individu, period, parameters):
        ...
        duree_min_titre_sejour = parameters(
            period
        ).prestations.minima_sociaux.rsa.duree_min_titre_sejour
        return or_(
            ressortissant_eee, duree_possession_titre_sejour >= duree_min_titre_sejour
        )
    ...
    def formula(individu, period, parameters):
        ...
        duree_min_titre_sejour = parameters(
            period
        ).prestations.minima_sociaux.rmi.duree_min_titre_sejour
        return or_(
            ressortissant_eee, duree_possession_titre_sejour >= duree_min_titre_sejour
        )
...
class rsa_eligibilite(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        condition_nationalite = famille.any(
            condition_nationalite_i, role=Famille.PARENT
        )
...
class rsa_eligibilite_tns(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        tns_benefice_agricole_i = famille.members(
        ...
        tns_autres_revenus_type_activite_i = famille.members(
            "tns_autres_revenus_type_activite", period
        )
        ...
        def eligibilite_agricole(
            ...
            taux_avec_conjoint = (
                1
                + maj_2p
                + maj_1e_2ad * (rsa_nb_enfants > 0)
                + maj_e_sup * max_(rsa_nb_enfants - 1, 0)
            )
            taux_sans_conjoint = (
                1
                + maj_2p * (rsa_nb_enfants > 0)
                + maj_e_sup * max_(rsa_nb_enfants - 1, 0)
            )
            taux_majoration = (
                has_conjoint * taux_avec_conjoint
                + (1 - has_conjoint) * taux_sans_conjoint
            )
            plafond_benefice_agricole_majore = (
                taux_majoration * plafond_benefice_agricole
            )
        ...
        def eligibilite_chiffre_affaire(ca, type_activite, P_micro):
            ...
            service = (type_activite == TypesTnsTypeActivite.bic) + (
                type_activite == TypesTnsTypeActivite.bnc
            )

            return (achat_revente * (ca <= plaf_vente)) + (
                service * (ca <= plaf_service)
            )
        ...
        eligibilite_chiffre_affaire = famille.all(
            eligibilite_chiffre_affaire(
                tns_autres_revenus_CA_i, tns_autres_revenus_type_activite_i, P_micro
            ),
            role=Famille.PARENT,
        )

        return (
            eligibilite_agricole * not_(tns_avec_employe) * eligibilite_chiffre_affaire
        )
...
class rsa_forfait_asf(Variable):
    ...
    def formula_2014_04_01(famille, period, parameters):
        ...
        montant_verse_par_enfant = (
            prestations_familiales.af.bmaf * prestations_familiales.asf.taux_1_parent
        )
        montant_retenu_rsa_par_enfant = (
            prestations_familiales.af.bmaf * minima_sociaux.rmi.forfait_asf.taux1
        )

        asf_retenue = asf_verse * (
            montant_retenu_rsa_par_enfant / montant_verse_par_enfant
        )
    ...
    def formula(famille, period, parameters):
        ...
        avantage_nature = or_(
            (
                ...
                + (
                    statut_occupation_logement
                    == TypesStatutOccupationLogement.proprietaire
                )
            )
            * not_(loyer),
            (
        ...
        if period.start.date >= date(2009, 6, 1):
            ...
            forf_logement_taux_3p = (
                params.forfait_logement.taux_3_personnes_ou_plus * taux_3p
            )
        else:
...
class rsa_non_calculable(Variable):
    ...
    def formula(famille, period):
        ...
        non_calculable_tns_parent1 = famille.demandeur(
            "rsa_non_calculable_tns_individu", period
        )
        non_calculable_tns_parent2 = famille.conjoint(
            "rsa_non_calculable_tns_individu", period
        )

        non_calculable = select(
            [non_calculable_tns_parent1, non_calculable_tns_parent2], [1, 2]
        )
        ...
        return select(
            ...
            [
                TypesRSANonCalculable.calculable,
                TypesRSANonCalculable.tns,
                TypesRSANonCalculable.conjoint_tns,
            ],
        )
...
class rsa_non_calculable_tns_individu(Variable):
    ...
    def formula(individu, period):
        ...
        tns_autres_revenus = individu(
            "tns_autres_revenus", this_year_and_last_year, options=[ADD]
        )
...
class rsa_socle(Variable):
    ...
    def formula_2009_06_01(famille, period, parameters):
        ...
        taux = (
            ...
            * where(
                nb_parents == 1,
                rsa.majoration_rsa.taux_personne_supp,
                rsa.majoration_rsa.taux_troisieme_personne,
            )
            + max_(  # Si nb_parents == 1, pas de conjoint, la 4e personne est un enfant, donc le taux est de 40%.
                nb_personnes - 4, 0
            )
            * rsa.majoration_rsa.taux_personne_supp
        )
