...
class ppa_eligibilite_etudiants(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        def condition_ressource(period2):
            revenu_activite = famille.members(
                "ppa_revenu_activite_individu", period2, extra_params=[period]
            )
        ...
        condition_famille = famille.any(
            not_(etudiant_i) + condition_etudiant_i, role=Famille.PARENT
        )
...
class ppa_montant_forfaitaire_familial_non_majore(Variable):
    ...
    def formula(famille, period, parameters, mois_demande):
        ...
        taux_non_majore = (
            ...
            + (nb_personnes >= 4)
            * where(
                nb_parents == 1, ppa.taux_personne_supp, ppa.taux_troisieme_personne
            )
            +
...
class ppa_montant_forfaitaire_familial_majore(Variable):
    ...
    def formula(famille, period, parameters, mois_demande):
        ...
        taux_majore = (
            ...
            + ppa.majoration_isolement_enf_charge * nb_enfants
        )
...
class ppa_revenu_activite_individu(Variable):
    ...
    def formula(individu, period, parameters, mois_demande):
        ...
        seuil_aah_activite = (
            P.prestations.minima_sociaux.ppa.seuil_aah_activite * smic_horaire
        )
        aah_activite = (revenus_activites >= seuil_aah_activite) * individu(
            "aah", period
        )
...
class ppa_ressources_hors_activite(Variable):
    ...
    def formula(famille, period, parameters, mois_demande):
        pf = famille(
            ...
            period,
            extra_params=[mois_demande],
        )
        ressources_hors_activite_i = famille.members(
            "ppa_ressources_hors_activite_individu", period, extra_params=[mois_demande]
        )
        ressources = ["ass", "asi", "aspa"]

        ressources_hors_activite = (
            famille.sum(ressources_hors_activite_i)
            + pf
            + sum(famille(ressource, mois_demande) for ressource in ressources)
        )
...
class ppa_ressources_hors_activite_individu(Variable):
    ...
    def formula(individu, period, parameters, mois_demande):
        ...
        aah_hors_activite = (revenus_activites < seuil_aah_activite) * individu(
            "aah", period
        )
...
class ppa_bonification(Variable):
    ...
    def formula(individu, period, parameters, mois_demande):
        ...
        seuil_1 = (
            P.prestations.minima_sociaux.ppa.bonification.seuil_bonification
            * smic_horaire
        )
        seuil_2 = (
            P.prestations.minima_sociaux.ppa.bonification.seuil_max_bonification
            * smic_horaire
        )
        bonification_max = round_(
            P.prestations.minima_sociaux.ppa.bonification.taux_bonification_max
            * ppa_base,
            2,
        )
        bonification = (
            bonification_max * (revenu_activite - seuil_1) / (seuil_2 - seuil_1)
        )
...
class ppa_forfait_logement(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        montant_base = famille(
            "ppa_montant_forfaitaire_familial_non_majore", period, extra_params=[period]
        )
...
class ppa_fictive(Variable):
    ...
    def formula(famille, period, parameters, mois_demande):
        ...
        montant_forfaitaire_familialise = where(
            ppa_majoree_eligibilite, mff_majore, mff_non_majore
        )
        ppa_base_ressources = famille(
            "ppa_base_ressources", period, extra_params=[mois_demande]
        )
        ppa_revenu_activite = famille(
            "ppa_revenu_activite", period, extra_params=[mois_demande]
        )
        bonification_i = famille.members(
            "ppa_bonification", period, extra_params=[mois_demande]
        )
...
class ppa(Variable):
    ...
    def formula_2016_01_01(famille, period, parameters):
        seuil_non_versement = parameters(
            period
        ).prestations.minima_sociaux.ppa.seuil_non_versement
