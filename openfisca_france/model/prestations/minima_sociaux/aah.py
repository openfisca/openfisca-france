...
class aah_base_ressources(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        def assiette_demandeur(revenus_demandeur):
            smic_brut_annuel = (
                12 * law.cotsoc.gen.smic_h_b * law.cotsoc.gen.nb_heure_travail_mensuel
            )
        ...
        def base_ressource_eval_trim():
            base_ressource_demandeur = famille.demandeur(
                "aah_base_ressources_eval_trimestrielle", period
            )
            base_ressource_conjoint = famille.conjoint(
                "aah_base_ressources_eval_trimestrielle", period
            )

            return assiette_demandeur(base_ressource_demandeur) + assiette_conjoint(
...
class aah_base_ressources_eval_trimestrielle(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        retraite_nette = individu(
            "retraite_nette", three_previous_months, options=[ADD]
        )
        pensions_alimentaires_percues = individu(
            "pensions_alimentaires_percues", three_previous_months, options=[ADD]
        )
        ...
        indemnites_journalieres_imposables = individu(
            "indemnites_journalieres_imposables", three_previous_months, options=[ADD]
        )
        indemnites_stage = individu(
            "indemnites_stage", three_previous_months, options=[ADD]
        )
        revenus_stage_formation_pro = individu(
            "revenus_stage_formation_pro", three_previous_months, options=[ADD]
        )
        ...
        prestation_compensatoire = individu(
            "prestation_compensatoire", three_previous_months, options=[ADD]
        )
        pensions_invalidite = individu(
            "pensions_invalidite", three_previous_months, options=[ADD]
        )
        indemnites_chomage_partiel = individu(
            "indemnites_chomage_partiel", three_previous_months, options=[ADD]
        )
        bourse_recherche = individu(
            "bourse_recherche", three_previous_months, options=[ADD]
        )
        gains_exceptionnels = individu(
            "gains_exceptionnels", three_previous_months, options=[ADD]
        )
        ...
        def revenus_tns():
            ...
            tns_micro_entreprise_benefice = (
                individu("tns_micro_entreprise_benefice", last_year) * 3 / 12
            )
            tns_benefice_exploitant_agricole = (
                individu("tns_benefice_exploitant_agricole", last_year) * 3 / 12
            )
...
class aah_base_ressources_eval_annuelle(Variable):
    ...
    def formula(individu, period, parameters):
        return individu("revenu_activite", period.n_2) + individu(
            "revenu_assimile_pension", period.n_2
        )
...
class aah_eligible(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        eligible_aah = (
            ...
            * (
                (age >= law.minima_sociaux.aah.age_minimal)
                + ((age >= 16) * (autonomie_financiere))
            )
        )
...
class TypesAAHNonCalculable(Enum):
    __order__ = (
        "calculable intervention_CDAPH_necessaire"
...
class aah_base(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        plaf_ress_aah = (
            12
            * law.minima_sociaux.aah.montant
...
class caah(Variable):
    ...
    def formula_2005_07_01(individu, period, parameters):
        ...
        al = individu.famille(
            "aide_logement_montant", period
        )  # montant allocs logement de la famille
        taux_incapacite = individu("taux_incapacite", period)

        elig_cpl = ((aah > 0) | (benef_asi > 0)) * (
            taux_incapacite >= law.minima_sociaux.aah.taux_incapacite
        )
