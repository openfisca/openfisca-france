...
class aah_base_ressources_deconjugalisee(Variable):
    ...
    def formula_2023_10_01(individu, period, parameters):
        P = parameters(period)
        aah = P.prestations_sociales.prestations_etat_de_sante.invalidite.aah

        en_activite = ((individu('salaire_imposable', period, options = [ADD]) + individu('rpns_imposables', period.last_year)) > 0)

        def assiette_revenu_activite_demandeur(revenus_demandeur):
            smic_brut_annuel = 12 * P.marche_travail.salaire_minimum.smic.smic_b_horaire * P.marche_travail.salaire_minimum.smic.nb_heures_travail_mensuel
            total_tranche1 = min_(aah.travail_ordinaire.tranche_smic * smic_brut_annuel, revenus_demandeur)
            total_tranche2 = max_(0, revenus_demandeur - total_tranche1)
            return (1 - aah.travail_ordinaire.abattement_30) * total_tranche1 + (1 - aah.travail_ordinaire.abattement_sup) * total_tranche2
...
class aah_eligible(Variable):
    ...
    def formula_2011_01_01(individu, period, parameters):
        P = parameters(period).prestations_sociales.prestations_etat_de_sante.invalidite.aah
        taux_incapacite = individu('taux_incapacite', period)
        taux_incapacite_max = (taux_incapacite >= P.taux_capacite.taux_incapacite)
        taux_incapacite_rsdae = (taux_incapacite >= P.taux_capacite.taux_incapacite_rsdae)
        ...
        eligible_aah = (
            (taux_incapacite_max + (taux_incapacite_rsdae * rsdae * (age <= P.age_legal_retraite)))
            * ((age >= P.age_minimal) + ((age >= P.age_fin_educ) * not_(prestations_familiales_enfant_a_charge)))
            )
...
class aah_plafond_ressources_conjugalise(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        return montant_max * (
            + 1
            + en_couple
            * P.prestations_etat_de_sante.invalidite.aah.majoration_plafond.majoration_plafond_couple
            + P.prestations_etat_de_sante.invalidite.aah.majoration_plafond.majoration_par_enfant_supplementaire
            * af_nbenf
            )
...
class aah(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        # montant_max_aah = P.prestations_etat_de_sante.invalidite.aah.montant
