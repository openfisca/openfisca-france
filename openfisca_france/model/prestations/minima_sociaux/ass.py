...
class ass(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        plafond_mensuel = montant_journalier * (
            ass_params.plaf_seul * not_(en_couple) + ass_params.plaf_coup * en_couple
        )
...
class ass_base_ressources(Variable):
    ...
    def formula(famille, period):
        ass_base_ressources_demandeur = famille.demandeur(
            "ass_base_ressources_individu", period
        )
        ass_base_ressources_conjoint = famille.conjoint(
            "ass_base_ressources_conjoint", period
        )
...
class ass_base_ressources_conjoint(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        def calculateWithAbatement(ressourceName, neutral_totale=False):
            ...
            tx_abat_applique = (
                abat_partiel * tx_abat_partiel + abat_total * tx_abat_total
            )

            return (1 - tx_abat_applique) * ressource_year

        salaire_imposable = calculateWithAbatement("salaire_imposable")
        indemnites_stage = calculateWithAbatement(
            "indemnites_stage", neutral_totale=True
        )
        ...
        def revenus_tns():
            ...
            tns_autres_revenus = individu(
                "tns_autres_revenus", last_year, options=[ADD]
            )
...
class ass_eligibilite_cumul_individu(Variable):
    ...
    def formula_2017_09_01(individu, period):
        ...
        for mois in douze_mois_precedents:
            ...
            nb_mois_consecutif_sans_activite = where(
                absence_ressources_activite * chomeur,
                nb_mois_consecutif_sans_activite + 1,
                0,
            )
...
class ass_eligibilite_individu(Variable):
    ...
    def formula_2017_01_01(individu, period):
        ...
        return and_(
            not_(aah_eligible),
            and_(demandeur_emploi_non_indemnise, ass_precondition_remplie),
        )
