...
class asi_aspa_base_ressources_individu(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        revenus_capitaux_prelevement_bareme_foyer_fiscal = max_(
            0,
        ...
        revenus_capitaux_prelevement_liberatoire_foyer_fiscal = max_(
            0,
            individu.foyer_fiscal(
        ...
        rente_viagere_titre_onereux_foyer_fiscal = individu.foyer_fiscal(
            "rente_viagere_titre_onereux", three_previous_months, options=[ADD]
        )
        revenus_foyer_fiscal = (
            revenus_capitaux_prelevement_bareme_foyer_fiscal
            + revenus_capitaux_prelevement_liberatoire_foyer_fiscal
            + rente_viagere_titre_onereux_foyer_fiscal
        )
        ...
        def revenus_tns():
            revenus_auto_entrepreneur = individu(
                "tns_auto_entrepreneur_benefice", three_previous_months, options=[ADD]
            )
        ...
        aah = (
            aah
            * not_(aspa_eligibilite)
            * not_(asi_eligibilite)
            * not_(pension_invalidite)
        )
        ...
        def abattement_salaire():
            ...
            abattement_forfaitaire = (
                abattement_forfaitaire_base * taux_abattement_forfaitaire
            )
            salaire_de_base = individu(
                "salaire_de_base", three_previous_months, options=[ADD]
            )

            return min_(salaire_de_base, abattement_forfaitaire)

        base_ressources_3_mois = (
            sum(
                max_(0, individu(ressource_type, three_previous_months, options=[ADD]))
                for ressource_type in ressources_incluses
            )
            + aah
            + revenus_foyer_fiscal_individu
            + revenus_tns()
            - abs_(pensions_alimentaires_versees)
            - abattement_salaire()
        )
...
class aspa_eligibilite(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        condition_invalidite = (
            taux_incapacite > P.aspa.taux_incapacite_aspa_anticipe
        ) + inapte_travail
        condition_age_base = age >= P.aspa.age_min
        condition_age_anticipe = (
            age >= P.aah.age_legal_retraite
        ) * condition_invalidite
...
class asi_aspa_condition_nationalite(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        duree_min_titre_sejour = parameters(
            period
        ).prestations.minima_sociaux.aspa.duree_min_titre_sejour
...
class asi(Variable):
    ...
    def formula_2007(famille, period, parameters):
        ...
        elig1 = (asi_aspa_nb_alloc == 1) & (
            demandeur_eligible_asi | conjoint_eligible_asi
        )
        ...
        elig4 = (
            (demandeur_eligible_asi & conjoint_eligible_aspa)
            | (conjoint_eligible_asi & demandeur_eligible_aspa)
        ) & maries
        ...
        montant_max = (
            ...
            + elig5 * (P.asi.montant_seul + P.aspa.montant_annuel_couple / 2)
        ) / 12
        ...
        plafond_ressources = (
            elig1
            * (
                P.asi.plafond_ressource_seul * not_(en_couple)
                + P.asi.plafond_ressource_couple * en_couple
            )
            + elig2 * P.asi.plafond_ressource_couple
            + elig3 * P.asi.plafond_ressource_couple
            + elig4 * P.aspa.plafond_ressources_couple
            + elig5 * P.aspa.plafond_ressources_couple
        ) / 12
...
class aspa(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        # Un seul éligible
        elig1 = (asi_aspa_nb_alloc == 1) & (
            demandeur_eligible_aspa | conjoint_eligible_aspa
        )
        # Couple d'éligibles
        ...
        plafond_ressources = (
            elig1
            * (
                P.aspa.plafond_ressources_seul * not_(en_couple)
                + P.aspa.plafond_ressources_couple * en_couple
            )
            + (elig2 | elig3 | elig4) * P.aspa.plafond_ressources_couple
        ) / 12

        depassement = max_(ressources - plafond_ressources, 0)

        diff = (elig1 | elig2) * (montant_max - depassement) + (elig3 | elig4) * (
            P.aspa.montant_annuel_couple / 12 / 2 - depassement / 2
        )
