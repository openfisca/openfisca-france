...
class cmu_acs_eligibilite(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        condition_independance = not_(
            a_charge_des_parents + habite_chez_parents + recoit_pension
        )
...
class acs_montant_i(Variable):
    ...
    def formula_2009_08_01(individu, period, parameters):
        ...
        montant_si_pac = select(
            [(age <= 15) * (age >= 0), age <= 25], [P.acs_moins_16_ans, P.acs_16_49_ans]
        )
        ...
        return where(
            individu.has_role(Famille.PARENT), montant_si_parent, montant_si_pac
        )
...
class cmu_forfait_logement_al(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        return (aide_logement > 0) * min_(
            12 * aide_logement, forfait_logement(nb_personnes_foyer, P, law_rsa)
        )
...
class cmu_c_plafond(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        rang_dans_fratrie = famille.members.get_rank(
            famille, -age_i, condition=is_enfant
        ...
        coeff_couple = (
            1 + cmu.coeff_p2 + famille.sum(coeff_enfant_i, role=famille.ENFANT)
        )
        ...
        plafonds = cmu.plafond_base * coefficient_dom * coefficient_famille
    ...
    def formula(individu, period, parameters):
        ...
        def abbattement_chomage():
            indemnites_chomage_partiel = individu(
                "indemnites_chomage_partiel", previous_year, options=[ADD]
            )
            salaire_net = individu("salaire_net", previous_year, options=[ADD])
            chomage_last_month = individu("chomage_net", last_month)
            condition = or_(
                chomage_last_month > 0, revenus_stage_formation_pro_last_month > 0
            )
        ...
        def neutralisation_stage_formation_pro():
            ...
            ) * revenus_stage_formation_pro_annee
...
class cmu_base_ressources(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        heberge_titre_gratuit = (
            statut_occupation_logement
            == TypesStatutOccupationLogement.loge_gratuitement
        )
        forfait_logement = (
            proprietaire + heberge_titre_gratuit
        ) * cmu_forfait_logement_base + cmu_forfait_logement_al

        ressources_individuelles = famille.members(
            "cmu_base_ressources_individu", period
        )
        ...
        ressources_enfants = famille.sum(
            ressources_individuelles * condition_enfant_a_charge, role=Famille.ENFANT
        )
...
class cmu_c(Variable):
    ...
    def formula(famille, period):
        ...
        if period.start.date >= date(2016, 1, 1):
            ...
            eligibilite_rsa = (rsa > 0) * (
                rsa_base_ressources < socle - rsa_forfait_logement
            )
...
def forfait_logement(nbp_foyer, P, law_rsa):
    ...
    montant_rsa_socle = law_rsa.rmi * (
        1 + law_rsa.txp2 * (nbp_foyer >= 2) + law_rsa.txp3 * (nbp_foyer >= 3)
    )
