...
class TypesEligibiliteANAH(Enum):
    __order__ = (
        "a_verifier modestes tres_modeste"
...
class eligibilite_anah(Variable):
    ...
    def formula(menage, period):
        ...
        in_idf = sum(
            [
                startswith(depcom, departement_idf)
                for departement_idf in departements_idf
            ]
        )

        rfr_declarants_principaux_du_menage = menage.members.has_role(
            FoyerFiscal.DECLARANT_PRINCIPAL
        ) * menage.members.foyer_fiscal("rfr", period.n_2)
        ...
        bareme_idf = select(
            ...
            [
                select(
                    [rfr <= 19875, rfr <= 24194],
                    [TypesEligibiliteANAH.tres_modeste, TypesEligibiliteANAH.modestes],
                    TypesEligibiliteANAH.a_verifier,
                ),
                select(
                    [rfr <= 29171, rfr <= 35510],
                    [TypesEligibiliteANAH.tres_modeste, TypesEligibiliteANAH.modestes],
                ...
                select(
                    [
                        rfr <= 46798 + ((nb_members - 5) * 5882),
                        rfr <= 56970 + ((nb_members - 5) * 7162),
                    ],
                    [2, 1],
                    0,
                ),
        ...
        bareme_out = select(
            ...
            [
                ...
                select(
                    [
                        rfr <= 33774 + ((nb_members - 5) * 4257),
                        rfr <= 43297 + ((nb_members - 5) * 5454),
                    ],
                    [TypesEligibiliteANAH.tres_modeste, TypesEligibiliteANAH.modestes],
                    TypesEligibiliteANAH.a_verifier,
                ),
