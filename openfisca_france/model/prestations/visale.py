...
class visale_eligibilite(Variable):
    ...
    def formula_2018_06_19(menage, period, parameters):
        ...
        eligibilite_age = majeur * (age <= parameters(period).prestations.aides_logement.visale.eligibilite.age_max)
        ...
        ressortissant_pays_eligible = sum([nationalite == str.encode(etat) for etat in parameters(period).prestations.aides_logement.visale.eligibilite.residence_hors_eee])  # TOOPTIMIZE: string encoding into bytes array should be done at load time
...
class visale_montant_max(Variable):
    ...
    def formula_2016_01_01(menage, period, parameters):
        ...
        minimum_etudiant = where(
            ...
            parameters(period).prestations.aides_logement.visale.plafond_loyer.etudiant.ile_de_france,
            parameters(period).prestations.aides_logement.visale.plafond_loyer.etudiant.hors_ile_de_france,
            )

        plafond_loyer = where(
            residence_ile_de_france,
            parameters(period).prestations.aides_logement.visale.plafond_loyer.cas_general.ile_de_france,
            parameters(period).prestations.aides_logement.visale.plafond_loyer.cas_general.hors_ile_de_france,
            )
...
class visale_base_ressources(Variable):
    ...
    def formula_2016_01_01(menage, period, parameters):
        ...
        revenus_familles_abattus = sum(
            ...
            ) * parameters(period).prestations.aides_logement.visale.quote_part_aides_logement
