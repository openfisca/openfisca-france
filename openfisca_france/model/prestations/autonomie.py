...
class apa_domicile_participation(Variable):
    ...
    def formula_2002(individu, period, parameters):
        ...
        seuil_inf = (
            parameters.apa_domicile.seuil_de_revenu_en_part_du_mtp.seuil_inferieur
        )
        seuil_sup = (
            parameters.apa_domicile.seuil_de_revenu_en_part_du_mtp.seuil_superieur
        )
        ...
        proratisation_couple = 1 + en_couple * (
            parameters.apa_domicile.divison_des_ressources_du_menage_pour_les_couples
            - 1
        )
        dependance_plan_aide_domicile_accepte = individu(
            "dependance_plan_aide_domicile_accepte", period
        )
        ...
        condition_ressources_domicile = [
            ...
            (seuil_inf * majoration_tierce_personne)
            < base_ressources_apa_domicile
            <= (seuil_sup * majoration_tierce_personne),
        ...
        return (
            select(condition_ressources_domicile, taux_participation)
            ...
            * (
                ...
                * (
                    ...
                    * base_ressources_apa_domicile
                    / (1.945 * majoration_tierce_personne)
                    + (
                        0.4 * 2.67 * majoration_tierce_personne
                        - 0.725 * majoration_tierce_personne
                    )
                    / (1.945 * majoration_tierce_personne)
                )
                + A_3
                * (
                    ...
                    * base_ressources_apa_domicile
                    / (1.945 * majoration_tierce_personne)
                    + (
                        0.2 * 2.67 * majoration_tierce_personne
                        - 0.725 * majoration_tierce_personne
                    )
                    / (1.945 * majoration_tierce_personne)
                )
...
class apa_domicile(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        seuil_non_versement = (
            parameters.seuil_de_versement_de_l_apa.seuil_versement.seuil_de_versement_de_l_apa
        )
        dependance_plan_aide_domicile_accepte = individu(
            "dependance_plan_aide_domicile_accepte", period
        )
...
class apa_etablissement(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        gir = individu("gir", period)
        base_ressources_apa = individu("base_ressources_apa", period)
        proratisation_couple_etablissement = 1 + en_couple * (
            parameters.apa_institution.divison_des_ressources_du_menage_pour_les_couples
            - 1
        )
        base_ressources_apa_etablissement = (
            base_ressources_apa / proratisation_couple_etablissement
        )
        dependance_tarif_etablissement_gir_5_6 = individu(
            "dependance_tarif_etablissement_gir_5_6", period
        )
        dependance_tarif_etablissement_gir_dependant = individu(
            "dependance_tarif_etablissement_gir_dependant", period
        )
        seuil_inf_inst = (
            parameters.apa_institution.seuil_de_revenu_en_part_du_mtp.seuil_inferieur
        )
        seuil_sup_inst = (
            parameters.apa_institution.seuil_de_revenu_en_part_du_mtp.seuil_superieur
        )
        majoration_tierce_personne = parameters.mtp.mtp
        ...
        conditions_ressources = [
            ...
            seuil_inf_inst * majoration_tierce_personne
            < base_ressources_apa_etablissement
            <= seuil_sup_inst * majoration_tierce_personne,
...
class apa_urgence_domicile(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        plafond_gir1 = (
            parameters.apa_domicile.plafond_de_l_apa_a_domicile_en_part_du_mtp.gir_1
        )
        part_urgence_domicile = (
            parameters.apa_domicile.apa_d_urgence.part_du_plafond_de_l_apa_a_domicile
        )
...
class apa_urgence_institution(Variable):
    ...
    def formula(individu, period, parameters):
        period = period.start.offset("first-of", "month").period("month")
        dependance_tarif_etablissement_gir_1_2 = individu(
            "dependance_tarif_etablissement_gir_5_6", period
        )
        part_urgence_institution = (
            parameters.apa_institution.apa_d_urgence.part_du_tarif_dependance_gir_1_2_de_l_etablissement_d_accueil
        )
        apa_urgence_institution = (
            part_urgence_institution * dependance_tarif_etablissement_gir_1_2
        )
...
class dependance_plan_aide_domicile_accepte(Variable):
    value_type = float
    label = (
        u"Coût du plan d'aide plafonné pris en compte pour la détermination de l'APA"
    )
    entity = Individu
    definition_period = MONTH
    reference = [
    ...
    def formula(individu, period, parameters):
        gir = individu("gir", period)
        dependance_plan_aide_domicile = individu(
            "dependance_plan_aide_domicile", period
        )
        parameters_autonomie = parameters(period.start).autonomie

        plafond_gir1 = (
            parameters_autonomie.apa_domicile.plafond_de_l_apa_a_domicile_en_part_du_mtp.gir_1
        )
        plafond_gir2 = (
            parameters_autonomie.apa_domicile.plafond_de_l_apa_a_domicile_en_part_du_mtp.gir_2
        )
        plafond_gir3 = (
            parameters_autonomie.apa_domicile.plafond_de_l_apa_a_domicile_en_part_du_mtp.gir_3
        )
        plafond_gir4 = (
            parameters_autonomie.apa_domicile.plafond_de_l_apa_a_domicile_en_part_du_mtp.gir_4
        )
