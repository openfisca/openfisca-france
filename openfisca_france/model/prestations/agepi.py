...
from openfisca_core.populations import DIVIDE, ADD
...
class contrat_de_travail_debut_agepi(Variable):
    value_type = date
    default_value = date(2013, 1, 1)
    entity = Individu
    label = "Date du 1er jour dans l'entreprise ou la formation pour le calcul de l'AGEPI"
...
class agepi_eligible(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        parents_isoles = famille('nb_parents', period, options = [DIVIDE]) == 1
        est_parent = famille.members.has_role(Famille.PARENT)

        age_enfants = famille.members('age', period, options = [DIVIDE])
        age_enfants_eligible = (age_enfants < parameters(period).prestations.agepi.age_enfant_maximum) * (age_enfants > 0)

        #  2

        agepi_non_percues = np.logical_not(famille.members('agepi_percue_12_derniers_mois', period , options = [DIVIDE]))
        ...
        reprises_activites_en_france = famille.members('emploi_ou_formation_en_france', period, options = [DIVIDE])

        #  5 FIXME

        date_demande_agepi = datetime.datetime
        ...
        date_debut_contrat_de_travail_plus_un_mois = famille.members('contrat_de_travail_debut_agepi', period.offset(1, 'month'))

        print(f"contrat_de_travail_debut_agepi: {famille.members('contrat_de_travail_debut_agepi', period)}")
        ...
        resident_en_mayotte = famille.members('reside_en_region_mayotte', period, options = [DIVIDE]) == 1
        ne_resident_pas_en_mayotte = np.logical_not(resident_en_mayotte)

        allocation_individu = famille.members('allocation_retour_emploi', period, options = [DIVIDE])
        ...
        duree_formation = famille.members('heures_remunerees_volume', period, options = [DIVIDE])
        ...
        periode_de_contrat_3_mois_minimum = famille.members('contrat_de_travail_duree', period, options = [DIVIDE]) >= 3
...
class agepi(Variable):
    ...
    def formula(famille, period, parameters):
        ...
        eligibilite_agepi = famille('agepi_eligible', period, options = [ADD])
