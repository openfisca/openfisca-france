...
class type_menage(Variable):
    ...
    def formula(menage, period):
        ...
        isole = not_(
            menage.personne_de_reference.famille("en_couple", period.first_month)
        )

        return (
            0 * (isole * (af_nbenf == 0))
            + 1 * (not_(isole) * (af_nbenf == 0))  # Célibataire
            + 2 * (not_(isole) * (af_nbenf == 1))  # Couple sans enfants
            + 3 * (not_(isole) * (af_nbenf == 2))  # Couple un enfant
            + 4 * (not_(isole) * (af_nbenf == 3))  # Couple deux enfants
            + 5 * (isole * (af_nbenf == 1))  # Couple trois enfants et plus
            + 6 * (isole * (af_nbenf == 2))  # Famille monoparentale un enfant
            + 7 * (isole * (af_nbenf == 3))  # Famille monoparentale deux enfants
...
class revenu_disponible(Variable):
    ...
    def formula(menage, period, parameters):
        ...
        ppe_i = menage.members.foyer_fiscal(
            "ppe", period
        )  # PPE du foyer fiscal auquel appartient chaque membre du ménage
        ppe = menage.sum(
            ppe_i, role=FoyerFiscal.DECLARANT_PRINCIPAL
        )  # On somme seulement pour les déclarants principaux
        ...
        )  # PF de la famille auquel appartient chaque membre du ménage
        prestations_sociales = menage.sum(
            prestations_sociales_i, role=Famille.DEMANDEUR
        )  # On somme seulement pour les demandeurs
...
class revenu_net(Variable):
    ...
    value_type = float
    reference = (
        u"http://impotsurlerevenu.org/definitions/115-revenu-net-imposable.php",
    )
    definition_period = YEAR
...
class revenu_initial_individu(Variable):
    ...
    def formula(individu, period):
        cotisations_employeur_contributives = individu(
            "cotisations_employeur_contributives", period
        )
...
class revenus_du_travail(Variable):
    ...
    def formula(individu, period):
        ...
        revenus_non_salaries = individu(
            "rpns", period, options=[ADD]
        )  #  TODO ou rpns_individu
...
class pensions(Variable):
    ...
    def formula(individu, period):
        ...
        rente_viagere_titre_onereux = foyer_fiscal(
            "rente_viagere_titre_onereux", period, options=[ADD]
        )
        pen_foyer_fiscal = pensions_alimentaires_versees + rente_viagere_titre_onereux
        pen_foyer_fiscal_projetees = pen_foyer_fiscal * (
            individu.has_role(foyer_fiscal.DECLARANT_PRINCIPAL)
        )
...
class revenus_du_capital(Variable):
    ...
    def formula(individu, period):
        ...
        revenus_capitaux_prelevement_bareme = foyer_fiscal(
            "revenus_capitaux_prelevement_bareme", period, options=[ADD]
        )
        ...
        revenus_foyer_fiscal_projetes = revenus_foyer_fiscal * individu.has_role(
            foyer_fiscal.DECLARANT_PRINCIPAL
        )
...
class minimum_vieillesse(Variable):
    ...
    def formula(famille, period):
        return famille("asi", period, options=[ADD]) + famille(
            "aspa", period, options=[ADD]
        )
...
class crds(Variable):
    ...
    def formula(individu, period):
        ...
        crds_foyer_fiscal = (
            crds_fon + crds_pv_mo + crds_pv_immo + crds_cap_bar + crds_cap_lib
        )
        crds_foyer_fiscal_projetee = crds_foyer_fiscal * individu.has_role(
...
class csg(Variable):
    ...
    def formula(individu, period):
        ...
        csg_deductible_salaire = individu(
            "csg_deductible_salaire", period, options=[ADD]
        )
        csg_imposable_chomage = individu("csg_imposable_chomage", period, options=[ADD])
        csg_deductible_chomage = individu(
            "csg_deductible_chomage", period, options=[ADD]
        )
        csg_imposable_retraite = individu(
            "csg_imposable_retraite", period, options=[ADD]
        )
        csg_deductible_retraite = individu(
            "csg_deductible_retraite", period, options=[ADD]
        )
        ...
        csg_foyer_fiscal_projetee = csg_foyer_fiscal * individu.has_role(
            FoyerFiscal.DECLARANT_PRINCIPAL
        )
...
class cotisations_non_contributives(Variable):
    ...
    def formula(individu, period):
        cotisations_employeur_non_contributives = individu(
            "cotisations_employeur_non_contributives", period, options=[ADD]
        )
        cotisations_salariales_non_contributives = individu(
            "cotisations_salariales_non_contributives", period, options=[ADD]
        )
...
class check_csk(Variable):
    ...
    def formula(menage, period):
        ...
        prel_foyer_fiscal_i = (
            prelsoc_cap_bar + prelsoc_pv_mo + prelsoc_fon
        ) * menage.members.has_role(FoyerFiscal.DECLARANT_PRINCIPAL)
...
class check_crds(Variable):
    ...
    def formula(menage, period):
        ...
        crds_foyer_fiscal_i = (
            crds_pv_mo + crds_fon + crds_cap_bar
        ) * menage.members.has_role(FoyerFiscal.DECLARANT_PRINCIPAL)
