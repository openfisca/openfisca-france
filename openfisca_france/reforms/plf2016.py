...
class plf2016(Reform):
    ...
    class decote(Variable):
        ...
        def formula_2014_01_01(foyer_fiscal, period, parameters):
            ...
            decote_celib = (ir_plaf_qf < plf.decote_seuil_celib) * (
                plf.decote_seuil_celib - .75 * ir_plaf_qf
            )
            decote_couple = (ir_plaf_qf < plf.decote_seuil_couple) * (
                plf.decote_seuil_couple - .75 * ir_plaf_qf
            )
            return (nb_adult == 1) * decote_celib + (nb_adult == 2) * decote_couple
...
class plf2016_counterfactual(Reform):
    ...
    class decote(Variable):
        ...
        def formula_2015_01_01(foyer_fiscal, period, parameters):
            ...
            return (
                (ir_plaf_qf < decote.seuil * inflator)
                * (decote.seuil * inflator - ir_plaf_qf)
                * 0.5
            )
    ...
    class reduction_impot_exceptionnelle(Variable):
        ...
        def formula_2015_01_01(foyer_fiscal, period, parameters):
            nb_adult = foyer_fiscal("nb_adult", period)
            nb_parents = foyer_fiscal.declarant_principal.famille(
                "nb_parents", period.first_month
            )
    ...
    def apply(self):
        for variable in [
            self.decote,
            self.reductions,
            self.reduction_impot_exceptionnelle,
        ]:
...
def counterfactual_2014_modify_parameters(parameters):
    ...
    parameters.impot_revenu.bareme[1].threshold.update(
        period=reform_period, value=6011 * inflator
    )
    parameters.impot_revenu.bareme[1].rate.update(
        period=reform_period, value=.055 * inflator
    )
    parameters.impot_revenu.bareme[2].threshold.update(
        period=reform_period, value=11991 * inflator
    )
    parameters.impot_revenu.bareme[2].rate.update(
        period=reform_period, value=.14 * inflator
    )
    parameters.impot_revenu.bareme[3].threshold.update(
        period=reform_period, value=26631 * inflator
    )
    parameters.impot_revenu.bareme[3].rate.update(
        period=reform_period, value=.30 * inflator
    )
    parameters.impot_revenu.bareme[4].threshold.update(
        period=reform_period, value=71397 * inflator
    )
    parameters.impot_revenu.bareme[4].rate.update(
        period=reform_period, value=.40 * inflator
    )
