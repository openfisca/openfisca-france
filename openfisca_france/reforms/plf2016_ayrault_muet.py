...
class ppe_elig_bis(Variable):
    ...
    def formula(foyer_fiscal, period, parameters):
        ...
        seuil = (veuf | celibataire_ou_divorce) * (ppe.seuil_rfr_eligibilite.personne_seule + 2 * max_(nbptr - 1, 0) * ppe.seuil_rfr_eligibilite.increment_par_demi_part)\
            + maries_ou_pacses * (ppe.seuil_rfr_eligibilite.couple_marie_pacse + 2 * max_(nbptr - 2, 0) * ppe.seuil_rfr_eligibilite.increment_par_demi_part)
