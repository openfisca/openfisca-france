...
def modify_parameters(parameters):
    ...
    reform_parameters_subtree = load_parameter_file(
        name="allocations_familiales_imposables", file_path=file_path
    )
...
class allocations_familiales_imposables(Reform):
    ...
    class rbg(Variable):
        ...
        def formula(foyer_fiscal, period, parameters):
            allocations_familiales_imposables = foyer_fiscal(
                "allocations_familiales_imposables", period, options=[ADD]
            )
    ...
    class rfr(Variable):
        label = (
            u"Nouveau revenu fiscal de référence intégrant les allocations familiales"
        )
        definition_period = YEAR
        ...
        def formula(foyer_fiscal, period, parameters):
            ...
            revenus_capitaux_prelevement_liberatoire = simulation.calculate_add(
