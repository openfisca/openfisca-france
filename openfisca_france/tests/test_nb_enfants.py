...
def test_nb_enfants():
    ...
    assert_near(nb_enf(simulation.famille, simulation.period.this_month, 3, 18), [2, 0])
    assert_near(nb_enf(simulation.famille, simulation.period.this_month, 19, 50), [0, 1]) # Adults don't count

    test_case['individus'][5]['autonomie_financiere'] = True
    simulation_2 = new_simulation(test_case)

    assert_near(nb_enf(simulation_2.famille, simulation_2.period.this_month, 19, 50), [0, 0])
