...
# foyer <- LoadIn(erfFoyFil)
    ...
    foyer = df.get_values(table="erFoyFil", variables=erfFoyVar)
# 
...
# fip <-data.frame(declar = foyer$declar)
    ...
    L = len(foyer.anaisenf).max()/5
    fip = DataFrame()
    fip['declar'] = foyer['declar']
# for (i in c(1:L)){
...
# }
#TODO: J'ai rien compris à la boucle ci-dessus
...
# fip$N <- row(fip)[,1]
# str(fip$N)
    fip = fip[fip['typ.1'] is not None]
    fip.pivot(index=["2":"18"])
...
# tyG <- upData(tyG,drop = c('N'))
    ...
    tyF = fip.xs('F', level='typ')
    tyG = fip.xs('G', level='typ')
...
# tyF['dup'] <- duplicated(tyF[,c("declar","naia")])
    tyF['dup'] = False
    tyG['dup'] = False
    tyF['dup'] = tyF[tyF.duplicated(cols=['declar', 'naia'])]
    tyG['dup'] = tyG[tyG.duplicated(cols=['declar', 'naia'])]

# tyFG <- join(tyF,tyG, by = c('declar','naia','dup'),type = 'right',match = 'first')
# iden <- tyFG$N
# rm(tyF,tyG,tyFG)
    tyFG = tyF.merge(tyG, on=['declar', 'naia', 'dup'])
    iden = tyFG
    del tyF, tyG, tyFG
...
# rm(tyH,tyI,tyHI,L)
    tyH = fip.xs('H', level='typ')
    tyI = fip.xs('I', level='typ')
    tyHI = tyH.merge(tyI, on=['declar', 'naia'])
