...
def process_tests_list(
    tests_list, monthly_amount=False, default_error_margin=1, forced_error_margin=None
):
    for test in tests_list:
        error_margin = (
            forced_error_margin
            if forced_error_margin
            else test.pop("error_margin", default_error_margin)
        )
        ...
        for variable, expected_value in test["output_vars"].items():
            calculated_value = simulation.calculate(variable).sum() / (
                1 * (not monthly_amount) + 12 * monthly_amount
            )
            yield check_calculation, variable, calculated_value, expected_value, error_margin


@nottest  # this function should not be considered as a test by nosetests
def simulation_from_test(
    test, monthly_amount=False, default_error_margin=1, forced_error_margin=None
):
    year = test["year"]
    ...
    simulation = (
        ...
        .init_single_entity(
            period=year, parent1=parent1, menage=menage, foyer_fiscal=foyer_fiscal
        )
        .new_simulation()
    )
...
def check_simulation_variable(
    description, simulation, variable, expected_value, error_margin
):
    calculated_value = simulation.calculate(variable).sum()
    yield check_calculation, variable, calculated_value, expected_value, error_margin
...
def zip_period_with_values(period_str, values):
    ...
    period_list = [
        str(period.start.period(period.unit).offset(index)) for index in range(size)
    ]
