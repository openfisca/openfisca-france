...
def new_simulation(test_case):
    return (
        ...
        .init_from_test_case(period=reference_period, test_case=test_case)
        .new_simulation()
    )
...
def test_transpose():
    ...
    af_foyer_fiscal = foyer_fiscal.first_person.famille(
        "af", period=reference_period, options=[ADD]
    )
...
def test_complex_chain_2():
    ...
    assert_near(
        simulation.famille.demandeur.menage.personne_de_reference(
            "age", period="2013-01"
        ),
        [30, 31, 33],
    )
    assert_near(
        simulation.famille.conjoint.menage.personne_de_reference(
            "age", period="2013-01"
        ),
        [31, 0, 0],
    )
    assert_near(
        simulation.famille.demandeur.foyer_fiscal.declarant_principal(
            "age", period="2013-01"
        ),
        [30, 32, 32],
    )
    assert_near(
        simulation.foyer_fiscal.conjoint.famille.demandeur("age", period="2013-01"),
        [30, 33],
    )
