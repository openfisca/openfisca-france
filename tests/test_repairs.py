...
def test_2_parents_2_enfants():
    ...
    assert_equal(simulation.calculate("age", period=janvier).tolist(), [40, 30, 10, 18])
    assert_equal(
        simulation.calculate("age_en_mois", period=janvier).tolist(),
        [40 * 12, 30 * 12, 10 * 12, 18 * 12],
    )
...
def test_famille_1_parent_3_enfants():
    ...
    scenario = tax_benefit_system.new_scenario().init_from_attributes(
        test_case=dict(
            familles=[dict(parents=[0], enfants=[1, 2, 3])],
            individus=[
                dict(),
                dict(),
    ...
    assert_equal(simulation.calculate("age", period=janvier).tolist(), [40, 10, 12, 18])
    assert_equal(
        simulation.calculate("age_en_mois", period=janvier).tolist(),
        [40 * 12, 10 * 12, 12 * 12, 18 * 12],
    )
...
def test_famille_2_parents_2_enfants():
    ...
    scenario = tax_benefit_system.new_scenario().init_from_attributes(
        test_case=dict(
            familles=[dict(parents=[0, 1], enfants=[2, 3])],
            individus=[
                dict(),
                dict(date_naissance=datetime.date(year - 30, 1, 1)),
                dict(),
...
def test_foyer_fiscal_1_declarant_3_personnes_a_charge():
    ...
    scenario = tax_benefit_system.new_scenario().init_from_attributes(
        test_case=dict(
            foyers_fiscaux=[dict(declarants=[0], personnes_a_charge=[1, 2, 3])],
            individus=[
                dict(),
                dict(),
                dict(date_naissance=datetime.date(year - 12, 1, 1)),
...
def test_foyer_fiscal_2_declarants_2_personnes_a_charge():
    ...
    scenario = tax_benefit_system.new_scenario().init_from_attributes(
        test_case=dict(
            foyers_fiscaux=[dict(declarants=[0, 1], personnes_a_charge=[2, 3])],
            individus=[
                dict(),
                dict(date_naissance=datetime.date(year - 30, 1, 1)),
                dict(),
...
def test_menage_1_personne_de_reference_3_enfants():
    ...
    scenario = tax_benefit_system.new_scenario().init_from_attributes(
        test_case=dict(
            ...
            menages=[dict(personne_de_reference=0, enfants=[1, 2, 3])],
        ),
        repair=True,
        year=year,
    )
...
def test_menage_1_personne_de_reference_1_conjoint_2_enfants():
    ...
    scenario = tax_benefit_system.new_scenario().init_from_attributes(
        test_case=dict(
            ...
            menages=[dict(personne_de_reference=0, conjoint=1, enfants=[2, 3])],
        ),
        repair=True,
        year=year,
    )
