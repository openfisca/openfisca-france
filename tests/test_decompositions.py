...
def check_decomposition_xml_file(file_path):
    ...
    decomposition_xml_json = conv.check(decompositionsxml.xml_decomposition_to_json)(
        decomposition_tree.getroot(), state=conv.default_state
    )

    decomposition_xml_json, errors = decompositionsxml.make_validate_node_xml_json(
        tax_benefit_system
    ...
    if errors is not None:
        ...
        if errors is None:
            raise ValueError(
                unicode(
                    json.dumps(decomposition_xml_json, ensure_ascii=False, indent=2)
                ).encode("utf-8")
            )
        raise ValueError(
            u"{0} for: {1}".format(
    ...
    decomposition_json, errors = decompositions.make_validate_node_json(
        tax_benefit_system
    ...
    if errors is not None:
        ...
        if errors is None:
            raise ValueError(
                unicode(
                    json.dumps(decomposition_json, ensure_ascii=False, indent=2)
                ).encode("utf-8")
            )
        raise ValueError(
            u"{0} for: {1}".format(
...
def test_decomposition_xml_files():
    files_path = [
        ...
        os.path.join(decompositions_directory, "fiche_de_paie_decomposition.xml"),
    ]
...
def test_decomposition_calculate():
    ...
    simulation = (
        ...
        .init_single_entity(period=year, parent1={})
        .new_simulation()
    )
